...
class CmSupplierCustomer(models.Model):
    _name = 'cm.supplier.customer'
    _description = 'Supplier/Customer Master'
    ...
    entry_type = fields.Selection([('customer', 'Customer'), ('supplier', 'Supplier')], 'Entry Type')
    ...
    update_user_id = fields.Many2one('res.users', string="Last Updated By", readonly=True, copy=False,
                                    ondelete='restrict')

    #Line Ref
    line_ids = fields.One2many('cm.supplier.customer.line', 'header_id', string='Additional Contact Details', copy=True)
    line_ids_a = fields.One2many('cm.supplier.customer.attachment.line', 'header_id', string='Attachment', copy=True)
    line_ids_b = fields.One2many('cm.supplier.customer.billing.address.line', 'header_id', string='Billing Address', copy=True)
    line_ids_c = fields.One2many('cm.supplier.customer.bank.details.line', 'header_id', string='Bank Details', copy=True)
    line_ids_d = fields.One2many('cm.supplier.customer.delivery.address.line', 'header_id', string='Delivery Address', copy=True)
    ...
    def name_validation(self):
        ...
        if self.name:
            ...
            from cm_supplier_customer where upper(REPLACE(name, ' ', ''))  = '%s'
    ...
    def short_name_validation(self):
        ...
        if self.short_name:
            ...
            from cm_supplier_customer where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s""" %(short_name, self.id))
            data = self.env.cr.dictfetchall()
            if len(data) > 0:
                raise UserError(_('Short name must be unique'))
        return True
    ...
    def _check_tin(self):
        if self.tin_no:
            rec_ids = self.env['cm.supplier.customer'].with_context(active_test=False).search([('tin_no', '=', self.tin_no), ('id', '!=', self.id)])
            if not rec_ids:
    ...
    def _check_pan_no(self):
        if self.pan_no:
            rec_ids = self.env['cm.supplier.customer'].with_context(active_test=False).search([('pan_no', '=', self.pan_no), ('id', '!=', self.id)])
            if not rec_ids:
                if len(self.pan_no) < 10 or len(self.pan_no) > 10:
    ...
    def _check_tan_no(self):
        if self.tan_no:
            rec_ids = self.env['cm.supplier.customer'].with_context(active_test=False).search([('tan_no', '=', self.tan_no), ('id', '!=', self.id)])
            if not rec_ids:
    ...
    def _check_vat(self):
        if self.vat_no:
            if len(str(self.vat_no)) == 15:
                return True
            else:
                raise UserError(_('Invalid VAT number. Please enter the correct VAT number'))
    ...
    def _check_gst_no(self):
        ...
        if self.gst_category == 'yes':
            if self.gst_no:
                ...
                self.env.cr.execute("""select upper(gst_no) from cm_supplier_customer where upper(REPLACE(gst_no, ' ', ''))  = '%s' """ %(gst_no))
                data = self.env.cr.dictfetchall()
                if len(data) > 1:
    ...
    def approve_warning_rule(self):
        ...
        if warning_msgs:
            ...
            context = {
                ...
                'transaction_model': 'cm.supplier.customer',
    ...
    def create(self, vals):
        ...
        return super(CmSupplierCustomer, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(CmSupplierCustomer, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        #counts
        cm_supplier_customer_master = self.env['cm.supplier.customer']
        result['all_draft'] = cm_supplier_customer_master.search_count([('status', '=', 'draft'),('entry_type', '=', 'customer')])
        result['all_active'] = cm_supplier_customer_master.search_count([('status', '=', 'active'),('entry_type', '=', 'customer')])
        result['all_inactive'] = cm_supplier_customer_master.search_count([('status', '=', 'inactive'),('entry_type', '=', 'customer')])
        result['all_editable'] = cm_supplier_customer_master.search_count([('status', '=', 'editable'),('entry_type', '=', 'customer')])
        result['my_draft'] = cm_supplier_customer_master.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid),('entry_type', '=', 'customer')])
        result['my_active'] = cm_supplier_customer_master.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid),('entry_type', '=', 'customer')])
        result['my_inactive'] = cm_supplier_customer_master.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid),('entry_type', '=', 'customer')])
        result['my_editable'] = cm_supplier_customer_master.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid),('entry_type', '=', 'customer')])
              
        result['all_today_count'] = cm_supplier_customer_master.search_count([('crt_date', '>=', fields.Date.today()),('entry_type', '=', 'customer')])
        result['all_month_count'] = cm_supplier_customer_master.search_count([('crt_date', '>=', datetime.today().replace(day=1)),('entry_type', '=', 'customer')])
        result['my_today_count'] = cm_supplier_customer_master.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today()),('entry_type', '=', 'customer')])
        result['my_month_count'] = cm_supplier_customer_master.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1)),('entry_type', '=', 'customer')])
    ...
    @api.model
    def supplier_retrieve_dashboard(self):
        """ This function returns the values to populate the custom dashboard in
            the transaction views.
        """
        ...
        #counts
        cm_supplier_customer_master = self.env['cm.supplier.customer']
        result['all_draft'] = cm_supplier_customer_master.search_count([('status', '=', 'draft'),('entry_type', '=', 'supplier')])
        result['all_active'] = cm_supplier_customer_master.search_count([('status', '=', 'active'),('entry_type', '=', 'supplier')])
        result['all_inactive'] = cm_supplier_customer_master.search_count([('status', '=', 'inactive'),('entry_type', '=', 'supplier')])
        result['all_editable'] = cm_supplier_customer_master.search_count([('status', '=', 'editable'),('entry_type', '=', 'supplier')])
        result['my_draft'] = cm_supplier_customer_master.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid),('entry_type', '=', 'supplier')])
        result['my_active'] = cm_supplier_customer_master.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid),('entry_type', '=', 'supplier')])
        result['my_inactive'] = cm_supplier_customer_master.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid),('entry_type', '=', 'supplier')])
        result['my_editable'] = cm_supplier_customer_master.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid),('entry_type', '=', 'supplier')])
              
        result['all_today_count'] = cm_supplier_customer_master.search_count([('crt_date', '>=', fields.Date.today()),('entry_type', '=', 'supplier')])
        result['all_month_count'] = cm_supplier_customer_master.search_count([('crt_date', '>=', datetime.today().replace(day=1)),('entry_type', '=', 'supplier')])
        result['my_today_count'] = cm_supplier_customer_master.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today()),('entry_type', '=', 'supplier')])
        result['my_month_count'] = cm_supplier_customer_master.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1)),('entry_type', '=', 'supplier')])
