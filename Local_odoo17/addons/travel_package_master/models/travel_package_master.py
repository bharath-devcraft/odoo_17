...
class organizer_bank_details(models.Model):
    """ Organizer Bank Details """
    _name = "organizer_bank_details.organizer_bank_details"
    _description = "Organizer Bank Details"
    ...
    name = fields.Many2one(
        ...
        readonly=True,
        default=lambda self: self.env.user.company_id)
    org_account_no = fields.Char('Account No')
    org_bank = fields.Char('Bank Name')
    org_account_holder_name =  fields.Char('Account Holder Name')
    org_branch_name =  fields.Char('Branch Name')
    org_bank_ifsc_code =  fields.Char('IFSC Code')
    org_upi_id =  fields.Char('UPI Id')
    ...
    def write(self, vals):
        ...
        vals.update({'update_date': time.strftime('%Y-%m-%d %H:%M:%S'),
                     'update_user_id': self.env.user.id})
        return super(organizer_bank_details, self).write(vals)
