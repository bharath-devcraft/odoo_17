...
class FieldsDatabank(models.Model):
    ...
    #Char
    name = fields.Char(string="Order No", readonly=True, index=True, copy=False, size=15)
    ...
    update_user_id = fields.Many2one(RES_USERS, string="Last Update By", readonly=True, copy=False,
                                    ondelete='restrict')
    attach_user_id = fields.Many2one(RES_USERS, string="Attached By", readonly=True, copy=False,
                                    ondelete='restrict')
    ...
    department_id = fields.Many2one(CM_MASTER, string="Department", tracking=True, 
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    brand_id = fields.Many2one(CM_MASTER, string="Brand", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    model_id = fields.Many2one(CM_MASTER, string="Model Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    catg_id = fields.Many2one(CM_MASTER, string="Category Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    del_term_id = fields.Many2one(CM_MASTER, string="Delivery Term", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    inward_id = fields.Many2one(CM_MASTER, string="Inward Type",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    outward_id = fields.Many2one(CM_MASTER, string="Outward Type",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    trans_type = fields.Many2one(CM_MASTER, string="Transaction Type",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    city_id = fields.Many2one(CM_MASTER, string="City",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    state_id = fields.Many2one('res.country.state', string="State", ondelete='restrict')
    country_id = fields.Many2one('res.country', string="Country", ondelete='restrict')
    division_id = fields.Many2one(CM_MASTER, string="Division", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    project_id = fields.Many2one(CM_MASTER, string="Project Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    bank_id = fields.Many2one(CM_MASTER, string="Bank Name", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    currency_id = fields.Many2one('res.currency', string="Currency", readonly=True,
                                 copy=False, tracking=True, ondelete='restrict')
    period_id = fields.Many2one(CM_MASTER, string="Period",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    account_id = fields.Many2one(CM_MASTER, string="Account Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    transport_id = fields.Many2one(CM_MASTER, string="Transport Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    employee_id = fields.Many2one(CM_MASTER, string="Employee Name", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    job_id = fields.Many2one(CM_MASTER, string="Designation",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    executive_id = fields.Many2one(CM_MASTER, string="Executive", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    segment_id = fields.Many2one(CM_MASTER, string="Segment",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    source_id = fields.Many2one(CM_MASTER, string="Source Location",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    destination_id = fields.Many2one(CM_MASTER, string="Destination Location", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    labor_id = fields.Many2one(CM_MASTER, string="Labor Name",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    branch_id = fields.Many2one(CM_MASTER, string="Branch Name", tracking=True,
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    expense_id = fields.Many2one(CM_MASTER, string="Expense",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    tax_group_id = fields.Many2one(CM_MASTER, string="Tax Group",
               ondelete='restrict', domain=[('status', 'in', ['active']),('active_trans', '=', True)])
    task_manager_id = fields.Many2one(RES_USERS, string="Task Manager", tracking=True, 
                                 ondelete='restrict')
    ...
    fy_control_date = fields.Date(string="FY Control Date")
    ...
    def validations(self) -> None:
        if 'entry_date' in self._fields:
            return self.env['fields.databank'].search([('entry_date','>',self.entry_date)])
