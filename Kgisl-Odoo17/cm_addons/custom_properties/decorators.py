...
def is_mobile_num(char: str) -> bool:
    """check the mobile number"""
    return True if re.match("^((\+)?\d{10,15})$", char) == None else False
...
def check_delete_access(user, record) -> bool:
    ...
    return True if ((user.id == record.user_id.id\
		or record.env[IR_CONFIG_PARAMETER].sudo().get_param('custom_properties.del_draft_entry')\
    ...
    def wrapper(*args, **kwargs) -> None:
        ...
        if self:
            if self.env[IR_CONFIG_PARAMETER].sudo().get_param('custom_properties.server_side_validation'):
                if 'delivery_date' in self._fields and 'entry_date' in self._fields and self.delivery_date and self.entry_date:
                ...
                if 'bank_name' in self._fields and self.bank_name:
                    if is_special_char(self.env, self.bank_name):
                       warning_msg.append(f"{specail_char_warning} {self._fields.get('bank_name').string}")
                if 'acc_holder_name' in self._fields and self.acc_holder_name:
                    if is_special_char(self.env, self.acc_holder_name):
                       warning_msg.append(f"{specail_char_warning} {self._fields.get('acc_holder_name').string}")
                if 'landmark' in self._fields and self.landmark:
                    if is_special_char(self.env, self.landmark):
                       warning_msg.append(f"{specail_char_warning} {self._fields.get('landmark').string}")
                ...
                if 'phone_no' in self._fields and self.phone_no and is_mobile_num(self.phone_no):
                   warning_msg.append(f"{num_only_warning} {self._fields.get('phone_no').string}")
                #Digits
                if 'adhar_no' in self._fields and self.adhar_no and is_number(self.adhar_no):
                   warning_msg.append(f"{num_only_warning} {self._fields.get('adhar_no').string}")
                #Age
