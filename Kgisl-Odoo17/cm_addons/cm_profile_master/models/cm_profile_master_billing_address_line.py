...
class CmProfileMasterBillingAddressLine(models.Model):
    ...
    def name_validation(self):
        """ name_validation """
        if self.name and is_special_char(self.env,self.name):
            raise UserError(_('Special character is not allowed in name field in billing address tab , Ref : %s')% self.name)
    
    @api.constrains('short_name')
    def short_name_validation(self):
        """ short_name_validation """
        if self.short_name and is_special_char(self.env,self.short_name):
            raise UserError(_('Special character is not allowed in short name field in billing address tab , Ref : %s')% self.short_name)
    
    @api.constrains('street')
    def _street_check(self):
        if self.street and is_special_char(self.env,self.street):
            raise UserError(_('Special character is not allowed in street field in billing address tab, Ref : %s')% self.street)
        return True                

    @api.constrains('street1')
    def _street1_check(self):
        if self.street1 and is_special_char(self.env,self.street1):
            raise UserError(_('Special character is not allowed in street1 field in billing address tab, Ref : %s')% self.street1)
        return True

    @api.constrains('phone_no')
    def phone_validation(self):
        if not(len(str(self.phone_no)) in (9, 10, 11, 12) and self.phone_no.isdigit() == True):
           raise UserError(_('Phone number is invalid. Please enter the correct phone number with SDD code in billing address tab, Ref : %s')% self.phone_no)
        return True

    @api.constrains('email')
    def _check_email(self):
        if self.email and is_valid_mail(self.email):
            raise UserError(_('Email is invalid. Please enter the correct email in billing address tab, Ref : %s')% self.email)
        return True
    ...
    def _check_gst_no(self):
        if self.gst_no:
            if len(str(self.gst_no)) == 15:
                ...
                raise UserError(_('Invalid GST number. Please enter the correct GST number in billing address tab, Ref : %s')% self.gst_no)
        ...
        if self.pincode:
            if self.country_id.code == 'IN':
                if len(str(self.pincode)) == 6 and self.pincode.isdigit() == True:
                    ...
                    raise UserError(_('Invalid zip code. Please enter the correct 6 digit zip code in billing address tab, Ref : %s')% self.pincode)
            else:
                if len(str(self.pincode)) not in (6, 7, 8, 9, 10):
                    raise UserError(_('Invalid zip code. Please enter the correct zip code in billing address tab, Ref : %s')% self.pincode)
                if is_special_char(self.env,self.pincode):
                    raise UserError(_('Special character is not allowed in zip code field in billing address tab, Ref : %s')% self.pincode)
    ...
    def _check_effective_date(self):
        if self.effective_date:
            ...
            from cm_profile_master_billing_address_line where effective_date='%s' 
            and header_id=%s """ %(self.effective_date, self.header_id.id))
            billing_add_data = self.env.cr.dictfetchall()
            if len(billing_add_data) > 1:
                raise UserError(_('Multiple billing address with same effective from date is not allowed'))
        ...
        if self.cin_no:
            if len(str(self.cin_no)) == 21:
                if is_special_char(self.env,self.cin_no):
                    raise UserError(_('Special character is not allowed in CIN number field in billing address tab, Ref : %s')% self.cin_no)
                cin_no_space = ''.join(c for c in self.cin_no if  c in ' ')
                if cin_no_space:
                    raise UserError(_('Space is not allowed in CIN number field in billing address tab, Ref : %s')% self.cin_no)
            else:
                raise UserError(_('Invalid CIN number. Please enter the correct CIN number in billing address tab, Ref : %s')% self.cin_no)
        return True
        
    @api.constrains('website')
    def _check_website(self):
        if self.website and re.match(
            r'www\.[\w-]{2,255}\.\w{2,6}(?:/\S*)?', self.website) is None:
            raise UserError(_('Website is invalid. Please enter the correct website in billing address tab, Ref : %s')% self.website)
        return True
