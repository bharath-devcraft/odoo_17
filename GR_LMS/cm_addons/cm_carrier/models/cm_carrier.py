# -*- coding: utf-8 -*-
import time
from odoo.addons.custom_properties.decorators import validation, is_special_char, valid_mobile_no, valid_phone_no, valid_email, valid_pin_code, valid_gst_no, valid_cin_no, valid_website
...
DIGITAL_SERVICE_NAME_OPTION = [('mobile_app', 'Mobile App'), ('api', 'API')]


class CmCarrier(models.Model):
    _name = 'cm.carrier'
    ...
    #Carrier Information
    parent_company_id = fields.Many2one('cm.carrier', string="Parent Company", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)], tracking=True)
    carrier_type = fields.Selection(selection=CARRIER_TYPE_OPTIONS, string="Carrier Type", copy=False)
    head_off_id = fields.Many2one('res.country', string="Headquarters", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)], tracking=True)
    oper_region_id = fields.Many2one('cm.region.master', string="Operating Region", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    ...
    line_ids = fields.One2many('cm.carrier.line', 'header_id', string="Additional Contact Details", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_carrier where upper(REPLACE(name, ' ', ''))  = '%s' 
            and id != %s and company_id = %s""" %(name, self.id,self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Carrier name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_carrier where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Carrier short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmCarrier, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_draft'] = cm_carrier.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_carrier.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_carrier.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_carrier.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_carrier.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_carrier.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_carrier.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_carrier.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_carrier.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_carrier.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_carrier.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_carrier.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
