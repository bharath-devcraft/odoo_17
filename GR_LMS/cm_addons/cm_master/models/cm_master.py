...
class CmMaster(models.Model):
    _name = 'cm.master'
    _description = 'Master Template'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'name asc'


    name = fields.Char(string="Name", index=True,filter_domain="[('cont_no','ilike',self)]", copy=False)
    ...
    line_ids = fields.One2many('cm.master.line', 'header_id', string="Details", copy=True, c_rule=True)
    line_ids_a = fields.One2many('cm.master.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
