...
class CmDepotVendorMasterBillingAddressLine(models.Model):
    _name = 'cm.depot.vendor.master.billing.address.line'
        ...
        for line in self:
            if line.pin_code:
                if line.country_id.code == 'IN' and  not(len(str(line.pin_code)) == 6 and line.pin_code.isdigit() == True):
                    raise UserError(_(f"Invalid zip code(IN). Please enter the correct 6 digit zip code in billing address tab, Ref : {line.pin_code}") )
                else:
                    if not valid_pin_code(line.pin_code):
                        raise UserError(_(f"Invalid zip code. Please enter the correct zip code in billing address tab, Ref : {line.pin_code}") )
