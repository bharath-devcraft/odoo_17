...
CM_DEPOT_VENDOR_MASTER='cm.depot.vendor.master'
CM_CITY = 'cm.city'
...
class CmDepotVendorMaster(models.Model):
    _name = 'cm.depot.vendor.master'
    _description = 'Depot Vendor Master'
    ...
    line_ids = fields.One2many('cm.depot.vendor.master.line', 'header_id', string="Additional Contacts", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_depot_vendor_master where upper(REPLACE(name, ' ', ''))  = '%s' 
            and id != %s and company_id = %s""" %(name, self.id,self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Depot vendor master name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_depot_vendor_master where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Depot vendor master short name must be unique"))
    ...
    def pan_no_validation(self):
        if self.pan_no:
            ...
            existing_record = self.env[CM_DEPOT_VENDOR_MASTER].search_count([('pan_no', '=', self.pan_no),('id', '!=', self.id), ('company_id', '=', self.company_id.id)])
            if existing_record:
    ...
    def write(self, vals):
        ...
        return super(CmDepotVendorMaster, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_draft'] = cm_depot_vendor_master.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_depot_vendor_master.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_depot_vendor_master.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_depot_vendor_master.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_depot_vendor_master.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_depot_vendor_master.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_depot_vendor_master.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_depot_vendor_master.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_depot_vendor_master.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_depot_vendor_master.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_depot_vendor_master.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_depot_vendor_master.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
