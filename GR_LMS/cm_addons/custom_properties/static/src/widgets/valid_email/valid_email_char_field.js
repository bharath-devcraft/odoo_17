...
export class CharField extends Component {
    static template = "case_sensitive_widget.ValidEmail_CharField";
    static components = {
        TranslationButton,
    };
    static props = {
    ...
    if (input.getAttribute('type') === 'text') {
        ...
        if (input.value.replace(/[^a-zA-Z0-9@._%+-]/g, '') !== input.value) {
            this.showErrorMessage('Special characters are not allowed, except @._%+-');
        }
        
        let newValue = input.value.toLowerCase().replace(/[^a-zA-Z0-9@._%+-]/g, '');
...
registry.category("fields").add("valid_email", charField);
