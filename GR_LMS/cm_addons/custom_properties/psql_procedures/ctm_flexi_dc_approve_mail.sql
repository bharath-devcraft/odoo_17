-- FUNCTION: public.ctm_flexi_dc_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_dc_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_dc_approve_mail(
	v_trans_id integer,
...
v_accessories_req='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			select ser.sys_ref into v_service_code from ct_delivery_challan trans
			...
			Open cursor_1 FOR
				...
				trans.cus_bill_address as cus_bill_address,
				trans.cus_del_address as cus_del_address,
				...
				from  ct_delivery_challan trans
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_service_name,v_customer_name,
				v_cus_bill_address,v_cus_del_address,v_accessories_req;
			...
			v_data=v_data || '<table id="customers">
		     ...
		     v_data=v_data || '
			 	<tr>
					...
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border" style="word-break:break-all">'||v_cus_bill_address||'</td>
				</tr>
				...
				<tr>
					...
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border" style="word-break:break-all">'||v_cus_del_address||'</td>
				</tr>
			...
			elseif v_service_code in ('FLAS') then 
				SELECT prod.name->>'en_US' into v_dub_bag_name
					FROM ct_delivery_challan_acc_line flexi
			...
			select accessories into v_accessories_req from ct_delivery_challan where id = 5;
			...
			if v_service_code in ('FLAS') or (v_service_code in ('FLBS') and v_accessories_req = 'with_accessories') then
				...
				Open cursor_2 FOR
					...
					from ct_delivery_challan_acc_details_line flexi_details
					left join ct_delivery_challan_acc_line flexi on flexi.id = flexi_details.header_id
...
ALTER FUNCTION public.ctm_flexi_dc_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
