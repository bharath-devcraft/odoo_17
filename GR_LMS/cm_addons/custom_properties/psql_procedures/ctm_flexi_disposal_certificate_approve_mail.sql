-- FUNCTION: public.ctm_flexi_disposal_certificate_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_disposal_certificate_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_disposal_certificate_approve_mail(
	v_trans_id integer,
...
v_dc_no char varying (10000);
v_customer_name char varying (10000);
v_address char varying (10000);
v_vendor char varying (10000);
v_disposal_category char varying (10000);
v_certificate_no char varying (10000);
v_disposal_date char varying (10000);
v_disposal_loc char varying (10000);
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			Open cursor_1 FOR
			    ...
			    (case when trans.disposal_category = 'export_disposal' then 'Export Disposal'
					 when trans.disposal_category = 'expired_disposal' then 'Expired Disposal'
					 when trans.disposal_category = 'import_disposal' then 'Import Disposal'
				end) as disposal_category,
				trans.certificate_no,
				to_char(trans.disposal_date, 'dd/mm/yyyy') as disposal_date,
				trans.disposal_loc,
				prod.name->>'en_US',
				uom.name->>'en_US',
				trans.qty

				from  ct_disposal_certificate trans
				...
				left join product_template prod on prod.id = trans.flexi_bag_id
				left join uom_uom uom on uom.id = trans.uom_id

				where trans.id = v_trans_id;

			LOOP                 
		  		FETCH cursor_1 INTO v_bc_no,v_dc_no,v_customer_name,v_address,v_vendor,
					v_disposal_category,v_certificate_no,v_disposal_date,v_disposal_loc,
					v_bag_name,v_uom,v_qty;
			...
			if(v_disposal_category is null) then
				v_disposal_category='';
			end if;

			if(v_certificate_no is null) then
				v_certificate_no='';
			end if;
			
			if(v_disposal_date is null) then
				v_disposal_date='';
			end if;
			
			if(v_disposal_loc is null) then
				v_disposal_loc='';
			end if;
			...
			v_data=v_data || '<table id="customers">
		     ...
		     v_data=v_data || '
			 	<tr>
					...
					<td colspan="3" id="remove-right-border">Disposal Category</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_disposal_category||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">DC NO</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_dc_no||'</td>
					<td colspan="3" id="remove-right-border">Certificate No</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_certificate_no||'</td>
				</tr>
				...
				<tr>
					...
					<td colspan="3" id="remove-right-border">Disposal Date</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_disposal_date||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">Address</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border" style="word-break:break-all">'||v_address||'</td>
					<td colspan="3" id="remove-right-border">Disposal Place</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_disposal_loc||'</td>
				</tr>
				...
				<tr>
					<td colspan="3" id="remove-right-border">Bag Name</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_bag_name||'</td>
					<td colspan="3" id="remove-right-border">UOM</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_uom||'</td>
				</tr>
...
ALTER FUNCTION public.ctm_flexi_disposal_certificate_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
