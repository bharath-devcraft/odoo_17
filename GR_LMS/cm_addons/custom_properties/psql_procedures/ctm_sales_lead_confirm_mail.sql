...
v_customer_type char varying (10000);
...
v_add_services char varying(10000);
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			select trans.name,
			(case when trans.customer_type = 'new' then 'New'
				 when trans.customer_type = 'existing' then 'Existing / New Service'
				 else '' end) as customer_type,
			trans.customer_name,trans.contact_person,trans.mobile_no,
			trans.email,trans.address,enq.name,
			(select string_agg(name, ', ') from cm_service where id in(
				select cm_service_id from cm_service_ct_sales_lead_rel 
				where ct_sales_lead_id = v_trans_id)) as add_service,
			trans.product,
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_trans_name,v_customer_type,v_customer_name,v_contact_person,
				v_mobile_no,v_email,v_address,v_lead_source,v_add_services,v_prod_list,
			...
			if(v_add_services is null) then
				v_add_services='';
			end if;
			...
			v_data=v_data || '<table id="customers">
		     ...
		     v_data=v_data || '<tr>
				...
				<tr>
				<td colspan="3">Customer Type</td>
				<td colspan="3">'||v_customer_type||'</td>
				</tr>
				...
				<td colspan="3">'||v_add_services||'</td>
				</tr>
				...
				<tr>
				<td colspan="3">Next Follow Up Date</td>
