-- FUNCTION: public.ctm_sub_quotation_approval_mail(integer, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_sub_quotation_approval_mail(integer, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_sub_quotation_approval_mail(
	v_trans_id integer,
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			ap_name.name as approved_by,currency.name as currency from  ct_quotations trans
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_trans_name,v_entry_date,v_bkg_party,v_service_name,v_enquiry_no,v_net_amt,v_val_date,v_confirmed_by,v_approved_by,v_currency;
		   ...
		   elseif (v_trans_state = 'quotation_sent') then
			    v_table_heading= v_table_heading || '<tr style="border-left:1px solid #2f9780;border-right:1px solid #2f9780;"><td colspan="2" style="border:none;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>The <b>'||v_ref_no||'</b> has been approved and the details are given below.</span></td></tr>
				</table>';
		   else 
			...
			<th colspan="18" scope="colgroup" class="table-heading green"><b>Sub Quotation Details:</b></th>';
...
ALTER FUNCTION public.ctm_sub_quotation_approval_mail(integer, character, character, character)
    OWNER TO odoo;
