-- FUNCTION: public.ctm_flexi_eway_bill_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_eway_bill_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_eway_bill_approve_mail(
	v_trans_id integer,
...
v_dc_date char varying(10000);
v_bill_no char varying (10000);
v_validity_date char varying (10000);
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			Open cursor_1 FOR
				...
				select 
				dc.name as dc_no,
				to_char(trans.dc_date,'dd/mm/yyyy') as dc_date,
				trans.name as bill_no,
				to_char(trans.due_date,'dd/mm/yyyy') as validity_date

				from  ct_eway_bill trans
				left join ct_delivery_challan dc on dc.id = trans.dc_id

				where trans.id = v_trans_id;

			LOOP                 
		  		FETCH cursor_1 INTO v_dc_no,v_dc_date,v_bill_no,v_validity_date;
			...
			if(v_validity_date is null) then
				v_validity_date='';
			end if;
			...
			v_data= v_table_heading || '<tr style="border-left:1px solid #2f9780;border-right:1px solid #2f9780;"><td colspan="2" style="border:none;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>The <b>'||v_dc_no||'</b> has been generated and the details are given below.</span></td></tr>
				</table>';
				
			v_data=v_data || '<table id="customers">
				<th colspan="18" scope="colgroup" class="table-heading green"><b>E-Way Bill Details:</b></th>';
		     ...
		     v_data=v_data || '
			 	<tr>
					...
					<td colspan="3" id="remove-right-border">E-Way Bill No</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_bill_no||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">DC Date</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_dc_date||'</td>
					<td colspan="3" id="remove-right-border">Validity Date</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_validity_date||'</td>
				</tr>';
...
ALTER FUNCTION public.ctm_flexi_eway_bill_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
