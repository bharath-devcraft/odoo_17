-- FUNCTION: public.ctm_flexi_payment_voucher_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_payment_voucher_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_payment_voucher_approve_mail(
	v_trans_id integer,
...
v_tot_advance char varying (10000);
v_eng_payable_value char varying (10000);
v_mobile_no char varying (10000);
v_hod_remarks char varying (10000);
v_tot_expense char varying (10000);
v_acc_payable_value char varying (10000);
v_hod_name char varying (10000);
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			Open cursor_1 FOR
				...
				trans.tot_advance,
				trans.eng_payable_value,
				emp.name as service_engineer,
				trans.mobile_no,
				trans.hod_remarks,
				trans.tot_expense,
				trans.acc_payable_value,
				emp2.name as hod_name,
				currency.name as currency

				from  ct_payment_voucher trans
				left join ct_business_confirmation bc on bc.id = trans.bc_id
				left join ct_job_card jc on jc.id = trans.jc_id
				left join cm_service ser on ser.id = trans.service_id
				left join cm_employee emp on emp.id = trans.emp_id
				left join cm_employee emp2 on emp2.id = trans.emp_hod_id
				left join res_currency currency on currency.id = trans.currency_id

				where trans.id = v_trans_id;

			LOOP                 
		  		FETCH cursor_1 INTO v_bc_no,v_jc_no,v_service_name,v_tot_advance,
					v_eng_payable_value,v_service_engineer,v_mobile_no,
					v_hod_remarks,v_tot_expense,v_acc_payable_value,v_hod_name,v_currency;
			...
			if(v_tot_advance is null) then
				v_tot_advance='';
			end if;
			
			if(v_eng_payable_value is null) then
				v_eng_payable_value='';
			end if;
			...
			if(v_hod_remarks is null) then
				v_hod_remarks='';
			end if;
			
			if(v_tot_expense is null) then
				v_tot_expense='';
			end if;
			
			if(v_acc_payable_value is null) then
				v_acc_payable_value='';
			end if;
			
			if(v_hod_name is null) then
				v_hod_name='';
			end if;
			...
			v_data=v_data || '<table id="customers">
				<th colspan="18" scope="colgroup" class="table-heading green"><b>Voucher Details:</b></th>';
		     ...
		     v_data=v_data || '
			 	<tr>
					...
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_service_engineer||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">JC No</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_jc_no||'</td>
				...
				<tr>
					...
					<td colspan="3" id="remove-right-border">HOD Remarks</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_hod_remarks||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">HOD Name</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||v_hod_name||'</td>
					<td colspan="3" id="remove-right-border">Total Expenses</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||trim(TO_CHAR((v_tot_expense::numeric)::float, '99G99G99G99G99G99G990D99'))||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">Total Advance</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||trim(TO_CHAR((v_tot_advance::numeric)::float, '99G99G99G99G99G99G990D99'))||'</td>
					<td colspan="3" id="remove-right-border">Accounts Payable</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||trim(TO_CHAR((v_acc_payable_value::numeric)::float, '99G99G99G99G99G99G990D99'))||' '|| v_currency ||'</td>
				</tr>
				
				<tr>
					<td colspan="3" id="remove-right-border">Engineer Payable</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||trim(TO_CHAR((v_eng_payable_value::numeric)::float, '99G99G99G99G99G99G990D99'))||' '|| v_currency ||'</td>
...
ALTER FUNCTION public.ctm_flexi_payment_voucher_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
