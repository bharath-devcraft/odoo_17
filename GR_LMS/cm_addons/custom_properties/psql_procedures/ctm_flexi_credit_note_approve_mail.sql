-- FUNCTION: public.ctm_flexi_credit_note_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_credit_note_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_credit_note_approve_mail(
	v_trans_id integer,
...
v_total_value char varying (10000);
...
v_accessories_req='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			select ser.sys_ref into v_service_code from ct_credit_note trans
			...
			Open cursor_1 FOR
				...
				select 
				bc.name as bc_no,
				to_char(trans.bc_date, 'dd/mm/yyyy') as bc_date,
				ser.name as service_name,
				cus.name as customer_name,
				trans.net_amt as total_value,
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_bc_no,v_bc_date,v_service_name,v_customer_name,
				v_total_value,v_accessories_req,v_currency;
			...
			v_data=v_data || '<table id="customers">
				<th colspan="18" scope="colgroup" class="table-heading green"><b>Credit Note Details:</b></th>';
		    
		     v_data=v_data || '
			 	<tr>
					<td colspan="3" id="remove-right-border">Credit Note No</td>
				...
				<tr>
					...
					<td colspan="3" id="remove-right-border">Credit Note Value</td>
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border">'||trim(TO_CHAR((v_total_value::numeric)::float, '99G99G99G99G99G99G990D99'))||' '|| v_currency ||'</td>
				</tr>';

			if v_service_code in ('FLBS') then 
				v_data = v_data || '
					<tr>
						<td colspan="3" id="remove-right-border">Accessories</td>
						<td colspan="3" id="no-border">:</td>
			...
			elseif v_service_code in ('FLAS') then 
				SELECT prod.name->>'en_US' into v_dub_bag_name
					FROM ct_credit_note_acc_line flexi
			...
			select accessories into v_accessories_req from ct_credit_note where id = v_trans_id;
			...
			if v_service_code in ('FLAS') or (v_service_code in ('FLBS') and v_accessories_req = 'with_accessories') then
				...
				Open cursor_2 FOR
					...
					from ct_credit_note_acc_details_line flexi_details
					left join ct_credit_note_acc_line flexi on flexi.id = flexi_details.header_id
...
ALTER FUNCTION public.ctm_flexi_credit_note_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
