-- FUNCTION: public.ctm_flexi_customer_invoice_approve_mail(integer, character, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_flexi_customer_invoice_approve_mail(integer, character, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_flexi_customer_invoice_approve_mail(
	v_trans_id integer,
...
v_invoice_no char varying (10000);
v_bill_address char varying (10000);
...
v_data='';
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			select ser.sys_ref into v_service_code from ct_customer_invoice trans
			...
			Open cursor_1 FOR
				...
				par.name as approved_by,
				trans.net_amt as invoice_value,
				trans.name as invoice_no,
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_bc_no,v_service_name,v_customer_name,v_approved_by,v_invoice_value,
					v_invoice_no,v_bill_address,v_mode_of_payment,v_due_date,v_currency;
			...
			v_data=v_data || '<table id="customers">
		     ...
		     v_data=v_data || '
			 	<tr>
					...
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border" style="word-break:break-all">'||v_ref_no||'</td>
				</tr>
				...
				<tr>
					...
					<td colspan="3" id="no-border">:</td>
					<td colspan="3" id="remove-left-border" style="word-break:break-all">'||v_bill_address||'</td>
				</tr>
...
ALTER FUNCTION public.ctm_flexi_customer_invoice_approve_mail(integer, character, character, character, character)
    OWNER TO odoo;
