...
class CmAgentCommissionAttachmentLine(models.Model):
    _name = 'cm.agent.commission.attachment.line'
    _description = 'Attachments'
    _order = 'attach_date desc'

    header_id = fields.Many2one('cm.agent.commission', string="Header Ref",index=True, required=True, ondelete='cascade', c_rule=True)
    ...
    attach_date = fields.Datetime(string="Attached Date", copy=False, default=fields.Datetime.now, readonly=True)
    attach_user_id = fields.Many2one(RES_USERS, string="Attached By", copy=False, default=lambda self: self.env.user.id, ondelete='restrict', readonly=True)
