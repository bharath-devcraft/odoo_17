...
class CmTankTcode(models.Model):
    _name = 'cm.tank.tcode'
    _description = 'Tank T Code'
    ...
    line_ids = fields.One2many('cm.tank.tcode.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_tank_tcode where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Tank T Code name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_tank_tcode where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Tank T Code short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmTankTcode, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_tank_tcode = self.env[CM_TANK_TCODE]
        result['all_draft'] = cm_tank_tcode.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_tank_tcode.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_tank_tcode.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_tank_tcode.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_tank_tcode.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_tank_tcode.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_tank_tcode.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_tank_tcode.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_tank_tcode.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_tank_tcode.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_tank_tcode.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_tank_tcode.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
