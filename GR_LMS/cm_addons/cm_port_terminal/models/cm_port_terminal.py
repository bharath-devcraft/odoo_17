...
TERMINAL_TYPE = [('container_terminal','Container Terminal'),
                         ('break_bulk_terminal','Break-Bulk Terminal'),
                         ('neo_bulk_terminal', 'Neo-Bulk Terminal'),
                         ('oil_terminal', 'Oil Terminal')]

YES_OR_NO = [('yes', 'Yes'), ('no', 'No')]

class CmPortTerminal(models.Model):
    _name = 'cm.port.terminal'
    _description = 'Port Terminal'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'name asc'

    name = fields.Char(string="Name", index=True, copy=False)
    short_name = fields.Char(string="Short Name", copy=False, help="Maximum 4 char is allowed and will accept upper case only", size=6, c_rule=True)
    ...
    line_ids = fields.One2many('cm.port.terminal.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_port_terminal where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Port terminal name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_port_terminal where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Port terminal short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmPortTerminal, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_port_terminal = self.env[CM_PORT_TERMINAL]
        result['all_draft'] = cm_port_terminal.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_port_terminal.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_port_terminal.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_port_terminal.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_port_terminal.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_port_terminal.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_port_terminal.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_port_terminal.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)]) 
        result['all_today_count'] = cm_port_terminal.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_port_terminal.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_port_terminal.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_port_terminal.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
