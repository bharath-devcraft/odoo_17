...
class CmUom(models.Model):
	
	_name = "uom.uom"
	_inherit = ['uom.uom','mail.thread', 'mail.activity.mixin', 'avatar.mixin']
	...
	def write(self, vals):
		...
		return super(CmUom, self).write(vals)
	...
	def retrieve_dashboard(self):
		...
		cm_uom = self.env['uom.uom']
		result['all_draft'] = cm_uom.search_count([('status', '=', 'draft')])
		result['all_active'] = cm_uom.search_count([('status', '=', 'active')])
		result['all_inactive'] = cm_uom.search_count([('status', '=', 'inactive')])
		result['all_editable'] = cm_uom.search_count([('status', '=', 'editable')])
		result['my_draft'] = cm_uom.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
		result['my_active'] = cm_uom.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
		result['my_inactive'] = cm_uom.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
		result['my_editable'] = cm_uom.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
			  
		result['all_today_count'] = cm_uom.search_count([('crt_date', '>=', fields.Date.today())])
		result['all_month_count'] = cm_uom.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
		result['my_today_count'] = cm_uom.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
		result['my_month_count'] = cm_uom.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
