...
CM_PRODUCT_CATEGORY = 'product.category'
RES_USERS = 'res.users'
...
class CmProductCategory(models.Model):
	
	_name = "product.category"
	_inherit = ['product.category','mail.thread', 'mail.activity.mixin', 'avatar.mixin']
	...
	type = fields.Selection([('view','Parent Category'), ('item_link','Product Category')], 'Purpose', help="A category of the view type is a virtual category that can be used as the parent of another category to create a hierarchical structure.",)
	...
	def name_validation(self):
		if self.name:
			...
			from product_category where upper(REPLACE(name, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("Product Category name must be unique"))
	...
	def code_validation(self):
		if self.short_name:
			...
			from product_category where upper(REPLACE(short_name, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("Product Category short name must be unique"))    
	...
	def write(self, vals):
		...
		return super(CmProductCategory, self).write(vals)
	...
	def retrieve_dashboard(self):
		...
		cm_product_category = self.env[CM_PRODUCT_CATEGORY]
		result['all_draft'] = cm_product_category.search_count([('status', '=', 'draft')])
		result['all_active'] = cm_product_category.search_count([('status', '=', 'active')])
		result['all_inactive'] = cm_product_category.search_count([('status', '=', 'inactive')])
		result['all_editable'] = cm_product_category.search_count([('status', '=', 'editable')])
		result['my_draft'] = cm_product_category.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
		result['my_active'] = cm_product_category.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
		result['my_inactive'] = cm_product_category.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
		result['my_editable'] = cm_product_category.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
			  
		result['all_today_count'] = cm_product_category.search_count([('crt_date', '>=', fields.Date.today())])
		result['all_month_count'] = cm_product_category.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
		result['my_today_count'] = cm_product_category.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
		result['my_month_count'] = cm_product_category.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
