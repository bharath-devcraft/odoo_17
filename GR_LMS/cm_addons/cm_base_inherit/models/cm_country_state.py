...
CM_COUNTRY_STATE = 'res.country.state'
RES_USERS = 'res.users'
...
class CmCountryState(models.Model):
	
	_name = "res.country.state"
	_inherit = ['res.country.state','mail.thread', 'mail.activity.mixin', 'avatar.mixin']
	_description = "State"
	...
	line_ids = fields.One2many('cm.country.state.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
	...
	def name_validation(self):
		if self.name:
			...
			from res_country_state where upper(REPLACE(name, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("State name must be unique"))
	...
	def code_validation(self):
		if self.code:
			...
			from res_country_state where upper(REPLACE(code, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(code, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("State code must be unique"))    
	...
	def write(self, vals):
		...
		return super(CmCountryState, self).write(vals)
	...
	def retrieve_dashboard(self):
		...
		cm_country_state = self.env[CM_COUNTRY_STATE]
		result['all_draft'] = cm_country_state.search_count([('status', '=', 'draft')])
		result['all_active'] = cm_country_state.search_count([('status', '=', 'active')])
		result['all_inactive'] = cm_country_state.search_count([('status', '=', 'inactive')])
		result['all_editable'] = cm_country_state.search_count([('status', '=', 'editable')])
		result['my_draft'] = cm_country_state.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
		result['my_active'] = cm_country_state.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
		result['my_inactive'] = cm_country_state.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
		result['my_editable'] = cm_country_state.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
			  
		result['all_today_count'] = cm_country_state.search_count([('crt_date', '>=', fields.Date.today())])
		result['all_month_count'] = cm_country_state.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
		result['my_today_count'] = cm_country_state.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
		result['my_month_count'] = cm_country_state.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
