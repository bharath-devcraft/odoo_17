...
class CtVendorInvoicePricingDetailsLine(models.Model):
    _name = 'ct.vendor.invoice.pri.details.line'
    ...
    @api.onchange('chrg_head_id')
    def onchange_charge_head_id(self):
        if self.chrg_head_id:
            self.uom_id = self.chrg_head_id.uom_id.id
        else:
            self.uom_id = False