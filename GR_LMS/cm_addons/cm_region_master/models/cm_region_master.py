...
CM_REGION_MASTER = 'cm.region.master'
RES_USERS = 'res.users'
...
class CmRegionMaster(models.Model):
    _name = 'cm.region.master'
    ...
    short_name = fields.Char(string="Short Name", copy=False, help="Maximum 4 char is allowed and will accept upper case only")
    status = fields.Selection(selection=CUSTOM_STATUS, string="Status", copy=False, default="draft", readonly=True, store=True, tracking=True)
    inactive_remark = fields.Text(string="Inactive Remarks", copy=False)
    remarks = fields.Html(string="Remarks", copy=False, sanitize=False)
    country_ids = fields.Many2many('res.country', string="Country", ondelete='restrict')
    ...
    line_ids = fields.One2many('cm.region.master.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_region_master where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Region Master name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_region_master where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Region Master short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmRegionMaster, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_region_master = self.env[CM_REGION_MASTER]
        result['all_draft'] = cm_region_master.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_region_master.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_region_master.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_region_master.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_region_master.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_region_master.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_region_master.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_region_master.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_region_master.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_region_master.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_region_master.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_region_master.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
