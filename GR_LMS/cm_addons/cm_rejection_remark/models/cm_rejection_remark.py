...
CM_REJECTION_REMARK = 'cm.rejection.remark'
RES_USERS = 'res.users'
...
class CmRejectionRemark(models.Model):
    ...
    line_ids = fields.One2many('cm.rejection.remark.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def write(self, vals):
        ...
        return super(CmRejectionRemark, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_active'] = cm_rej_remark.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_rej_remark.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_rej_remark.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_rej_remark.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_rej_remark.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_rej_remark.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_rej_remark.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_rej_remark.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_rej_remark.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_rej_remark.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_rej_remark.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
