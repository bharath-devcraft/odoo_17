...
CT_PAYMENT_VOUCHER = 'ct.payment.voucher'
RES_USERS = 'res.users'
...
class CtPaymentVoucher(models.Model):
    _name = 'ct.payment.voucher'
    _description = 'Payment Voucher'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'entry_date desc,name desc'

    name = fields.Char(string="Voucher No", readonly=True, index=True, copy=False, size=30, c_rule=True)
    status = fields.Selection(selection=CUSTOM_STATUS, string="Status", copy=False, default="draft", readonly=True, store=True, tracking=True)
    entry_date = fields.Date(string="Voucher Date", copy=False, default=fields.Date.today)
    ...
    jc_date = fields.Date(string="JC Date", copy=False)
    emp_id = fields.Many2one('cm.employee', string="Service Engineer Name", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    emp_hod_id = fields.Many2one('cm.employee', string=" HOD Name", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    mobile_no = fields.Char(string="Mobile No", size=15, copy=False)
    email = fields.Char(string="Email", copy=False, size=252)

    tot_expense = fields.Float(string="Total Expense", store=True, compute='_compute_all_line')	
    tot_advance = fields.Float(string="Total Advance", store=True, compute='_compute_all_line')
    eng_payable_value = fields.Float(string="Engineer Payable Value", store=True, compute='_compute_all_line')
    acc_payable_value = fields.Float(string="Accounts Payable Value", store=True, compute='_compute_all_line')
    ...
    hod_remarks = fields.Text(string="HOD Remarks", copy=False)
    ...
    line_ids_a = fields.One2many('ct.payment.voucher.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    line_ids_b = fields.One2many('ct.payment.voucher.expense.details.line', 'header_id', string="Expense Details", copy=True, c_rule=True)
    line_ids_c = fields.One2many('ct.payment.voucher.advance.details.line', 'header_id', string="Advance Details", copy=True, c_rule=True)

    @api.depends('line_ids_b', 'line_ids_c')
    def _compute_all_line(self):
        for data in self:         
            data.tot_expense = sum(data.line_ids_b.mapped('value'))
            data.tot_advance = sum(data.line_ids_c.mapped('value'))
            if data.tot_advance > data.tot_expense:
                data.eng_payable_value = data.tot_advance - data.tot_expense
                data.acc_payable_value = False
            elif data.tot_expense > data.tot_advance:
                data.acc_payable_value = data.tot_expense - data.tot_advance
                data.eng_payable_value = False
            else:
                data.eng_payable_value = 0.00
                data.acc_payable_value = 0.00

    @api.onchange('line_ids_b')
    def onchange_currency_id(self):
        currency_ids = self.line_ids_b.mapped('currency_id.id')
        ...
        if self.status == 'wfa':
            ...
            if not self.name:
                sequence_id = self.env[IR_SEQUENCE].search(
                        [('code', '=', CT_PAYMENT_VOUCHER)], limit=1)
                if sequence_id:
                    self.env.cr.execute(
            ...
            self.flexi_payment_voucher_mail_data_design(
                trans_rec = self,
                mail_queue_name = 'Flexi Payment Voucher Approve Mail',
                subject = f"#payment-voucher# {self.service_id.name} - {self.name}",
                mail_config_name = 'Flexi Payment Voucher Approve Mail'
            )
    ...
    def write(self, vals):
        ...
        return super(CtPaymentVoucher, self).write(vals)

    def  get_default_mail_ids(self, **kw):
        mail_ids = {}
        trans_rec = self.env[CT_PAYMENT_VOUCHER].search([('id', '=', kw.get('trans_id', False))])
    ...
    def flexi_payment_voucher_mail_data_design(self, **kw):
        self.env.cr.execute(
                "SELECT ctm_flexi_payment_voucher_approve_mail(%s, %s, %s, %s, %s)",
        ...
        if trans_rec and data[0][0] and mail_queue_name and subject and mail_config_name:
            ...
            vals = self.env['cp.mail.configuration'].mail_config_mailids_data(
                mail_type=mail_type, model_name=CT_PAYMENT_VOUCHER, mail_name=mail_config_name)
    ...
    def retrieve_dashboard(self):
        ...
        ct_trans = self.env[CT_PAYMENT_VOUCHER]
