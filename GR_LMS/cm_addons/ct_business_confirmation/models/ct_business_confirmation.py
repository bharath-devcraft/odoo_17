...
CT_BUSINESS_CONFIRMATION = 'ct.business.confirmation'
RES_USERS = 'res.users'
...
IS_REGISTER =  [('registered','Registered'),
...
MODE_OF_PAYMENT = [('cash', 'Cash'), ('credit', 'Credit')]
...
class CtBusinessConfirmation(models.Model):
    _name = 'ct.business.confirmation'
    _description = 'Business Confirmation'
    ...
    qs_no = fields.Char(string="Quotation No", copy=False)
    ...
    cpcb_registered = fields.Selection(selection=IS_REGISTER, string="CPCB Registered", copy=False)
    ...
    sds_attach_ids = fields.Many2many(IR_ATTACHMENT, 'ct_business_confirmation_ir_attachment_sds_rel', 'ct_bc_id', 'ir_attachment_id', string="SDS Document", ondelete='restrict', check_company=True)

    service_ids = fields.Many2many(CM_SERVICE, string="Additional Services", ondelete='restrict', domain="[('status', '=', 'active'),('active_trans', '=', True),('id', '!=', service_id),('category', '=', 'additional')]")
    spl_req = fields.Text(string="Special Requirements", copy=False)

    qs_remark = fields.Text(string="Quotation Remark", copy=False)
    payment_mode = fields.Selection(selection=MODE_OF_PAYMENT, string="Mode Of Payment")
    ...
    operational_type = fields.Selection(selection=OPERATIONAL_TYPE, string="Operation Type" ,copy=False)
    ...
    issue_status = fields.Selection(selection=ISSUE_STATUS, string="Issue Status", copy=False, default="pending", compute='_compute_pending_qty', store=True)
    invoice_status = fields.Selection(selection=ISSUE_STATUS, string="Invoice Status", copy=False, default="pending", compute='_compute_invoice_amount', store=True)
    invoiced_amt = fields.Float(string="Invoiced Value")
    ...
    line_ids_a = fields.One2many('ct.business.confirmation.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    line_ids_c = fields.One2many('ct.business.confirmation.acc.line', 'header_id', string="Flexi Details", copy=True, c_rule=True)
    line_ids_e = fields.One2many('ct.business.confirmation.cus.pri.details.line', 'header_id', string="Pricing Details", copy=True, c_rule=True)
    line_ids_f = fields.One2many('ct.business.confirmation.terms.conditions.line', 'header_id', string="Terms and Conditions", copy=True, c_rule=True)
    line_ids_g = fields.One2many('ct.business.confirmation.tax.line', 'header_id', string="Tax Breakup", copy=True, c_rule=True)
    ...
    @api.depends('line_ids_c.pending_qty')
    def _compute_pending_qty(self):
        for record in self:
            if record.line_ids_c:
                record.issue_status = 'pending' if any(line.pending_qty > 0 for line in record.line_ids_c) else 'closed'

    @api.depends('invoiced_amt')
    def _compute_invoice_amount(self):
        for record in self:
            record.invoice_status = 'pending' if record.invoiced_amt < record.net_amt else 'closed'
    ...
    def sequence_no_validations(self, **kw):
        warning_msg = []
        action_code_map = {
            'approve': CT_BUSINESS_CONFIRMATION
        }
        ...
        if self.status == 'wfa':
            ...
            if not self.name:
                sequence_id = self.env[IR_SEQUENCE].search(
                        [('code', '=', CT_BUSINESS_CONFIRMATION)], limit=1)
                if sequence_id:
                    self.env.cr.execute(
            ...
            self.flexi_bc_mail_data_design(
                trans_rec = self,
                mail_queue_name = 'Flexi BC Approve Mail',
                subject = f"#new-business# {self.service_id.name} - {self.name}",
                mail_config_name = 'Flexi BC Approve Mail'
            )
    ...
    def write(self, vals):
        ...
        return super(CtBusinessConfirmation, self).write(vals)

    def  get_default_mail_ids(self, **kw):
        mail_ids = {}
        trans_rec = self.env[CT_BUSINESS_CONFIRMATION].search([('id', '=', kw.get('trans_id', False))])
    ...
    def flexi_bc_mail_data_design(self, **kw):
        self.env.cr.execute(
                "SELECT ctm_flexi_bc_approve_mail(%s, %s, %s, %s, %s)",
        ...
        if trans_rec and data[0][0] and mail_queue_name and subject and mail_config_name:
            ...
            vals = self.env['cp.mail.configuration'].mail_config_mailids_data(
                mail_type=mail_type, model_name=CT_BUSINESS_CONFIRMATION, mail_name=mail_config_name)
    ...
    def auto_business_confirmation_entry_creation(self, **kw):
        qs_rec = kw.get('qs_rec', '')

        if qs_rec and qs_rec.service_id.sys_ref in ('FLBS', 'FLAS', 'FLOS'):
            ...
            vendor_address = ", ".join(filter(None, [
                qs_rec.vendor_id.street,
                qs_rec.vendor_id.street1,
                qs_rec.vendor_id.city_id.name,
                qs_rec.vendor_id.state_id.name,
                qs_rec.vendor_id.country_id.name,
                qs_rec.vendor_id.pin_code
            ]))
            ...
            bc_model = qs_rec.env['ct.business.confirmation']
            bc_rec = bc_model.create({
                'qs_no': qs_rec.name,
                ...
                'cpcb_registered': qs_rec.bkg_party_id.cpcb_registered,
                'service_ids': qs_rec.service_ids,
                'spl_req': qs_rec.spl_req,
                'line_ids_e': price_rec,
                'line_ids_f': [(0, 0, line.copy_data()[0]) for line in qs_rec.line_ids_k],
                'line_ids_a': [(0, 0, line.copy_data()[0]) for line in qs_rec.line_ids_a],
                'qs_remark': qs_rec.remarks,
            ...
            bc_rec.onchange_currency_id()

            if bc_rec and 'FLBS' in qs_rec.combined_codes:
                bc_rec.write({
                    'vendor_id': qs_rec.vendor_id.id,
                    'accessories_req': qs_rec.accessories_req,
                    'bill_address': vendor_address,
                    'del_address': vendor_address,
                ...
                flexi_bag_ids = [(0, 0, {
                    ...
                    'qty': rec.bag_qty,
                    'pending_qty': rec.bag_qty,
                    'pod_services': qs_rec.pod_services,
                    'bag_req_date': qs_rec.bag_req_date,
                    'flexi_type': qs_rec.flexi_type,
                    'flexi_layer_type_id': qs_rec.flexi_layer_type_id.id,
                    'flexi_capacity_id': qs_rec.flexi_capacity_id.id,
                    'vendor_id': qs_rec.vendor_id.id,
                    'line_ids': [(0, 0, {
                ...
                }) for rec in qs_rec.line_ids_b]

                bc_rec.line_ids_c = flexi_bag_ids
            
            if bc_rec and 'FLAS' in qs_rec.combined_codes:
                acc_ids = [(0, 0, {
                    'flexi_type': rec.flexi_type,
                    'flexi_layer_type_id': rec.flexi_layer_type_id.id,
                    'flexi_capacity_id': rec.flexi_capacity_id.id,
                    'vendor_id': rec.vendor_id.id,
                    'qty': rec.accessory_set_qty,
                    'pending_qty': rec.accessory_set_qty,
                ...
                }) for rec in qs_rec.line_ids]

                bc_rec.line_ids_c = acc_ids
            
            if bc_rec and 'FLOS' in qs_rec.combined_codes:
                bc_rec.write({
                    'operational_type' : qs_rec.operational_type,
                    'flexi_stuff_qty' : qs_rec.flexi_stuff_qty,
                    'stuff_date' : qs_rec.stuff_date,
                    'stuff_address' : qs_rec.stuff_address
                })
    ...
    def retrieve_dashboard(self):
        ...
        ct_trans = self.env[CT_BUSINESS_CONFIRMATION]
