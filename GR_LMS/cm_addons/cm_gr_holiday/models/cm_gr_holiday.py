...
TYPE =  [('company','Company'),
		('agent', 'Agent'),
		('tank_operator', 'Tank Operator')]

class CmGrHoliday(models.Model):
	_name = 'cm.gr.holiday'
	_description = 'Gr Holiday'
	...
	tank_operator_id = fields.Many2one('cm.tank.operator', string="Tank Operator Name", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])	
	...
	line_ids = fields.One2many('cm.gr.holiday.line', 'header_id', string="Details", copy=True, c_rule=True)
	line_ids_a = fields.One2many('cm.gr.holiday.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
	...
	def name_validation(self):
		if self.name:
			...
			from cm_gr_holiday where upper(REPLACE(name, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("Gr Holiday name must be unique"))
	...
	def short_name_validation(self):
		if self.short_name:
			...
			from cm_gr_holiday where upper(REPLACE(short_name, ' ', ''))  = '%s'
			and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
			if self.env.cr.fetchone():
				raise UserError(_("Gr Holiday short name must be unique"))
	...
	def validations(self):
		...
		if not self.line_ids:
			warning_msg.append("System not allow to approve with empty holiday details")
	...
	def write(self, vals):
		...
		return super(CmGrHoliday, self).write(vals)
	...
	def retrieve_dashboard(self):
		...
		result['all_active'] = cm_gr_holiday.search_count([('status', '=', 'active')])
		result['all_inactive'] = cm_gr_holiday.search_count([('status', '=', 'inactive')])
		result['all_editable'] = cm_gr_holiday.search_count([('status', '=', 'editable')])
		result['my_draft'] = cm_gr_holiday.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
		result['my_active'] = cm_gr_holiday.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
		result['my_inactive'] = cm_gr_holiday.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
		result['my_editable'] = cm_gr_holiday.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
			  
		result['all_today_count'] = cm_gr_holiday.search_count([('crt_date', '>=', fields.Date.today())])
		result['all_month_count'] = cm_gr_holiday.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
		result['my_today_count'] = cm_gr_holiday.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
		result['my_month_count'] = cm_gr_holiday.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
