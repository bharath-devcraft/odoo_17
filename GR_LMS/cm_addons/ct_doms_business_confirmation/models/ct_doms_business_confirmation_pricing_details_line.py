...
class CtDomsBusinessConfirmationPricingDetailsLine(models.Model):
    _name = 'ct.doms.bus.con.pricing.details.line'
    ...
    q_currency_id = fields.Many2one('res.currency', string="currency ")    
    ...
    @api.depends('qty', 'markup_value', 'q_currency_id')
    def _compute_tot_amt(self):
        for line in self:
            if line.header_id.currency_id and line.q_currency_id.name:
                if line.header_id.currency_id.name == line.q_currency_id.name:
                    line.tot_amt = line.markup_value * line.qty
                else:
                    exchange_rate = self.env['cm.exchange.rate']
                    line.tot_amt = exchange_rate.convert_to_base_currency(line.header_id.currency_id.name, line.q_currency_id.name,  (line.markup_value * line.qty))['converted_value']    
                
    @api.depends('qty', 'unit_price', 'q_currency_id')
    def _compute_converted_cost_price(self):
        for line in self:
            if line.header_id.currency_id and line.currency_id.name:
                if line.header_id.currency_id.name == line.currency_id.name:
                    line.converted_cost_price = line.unit_price * line.qty                            
                else:
                    exchange_rate = self.env['cm.exchange.rate']
                    line.converted_cost_price = exchange_rate.convert_to_base_currency(line.header_id.currency_id.name, line.q_currency_id.name,  (line.unit_price * line.qty))['converted_value'] 
        ...
        for line in self:
            if line.header_id.currency_id and line.currency_id.name:            
                if line.header_id.currency_id.name == line.q_currency_id.name:
                    ...
                    exchange_rate = self.env['cm.exchange.rate']
                    line.conversion_rate = exchange_rate.convert_to_base_currency(line.header_id.currency_id.name, line.q_currency_id.name, (line.markup_value * line.qty))['exchange_rate']
