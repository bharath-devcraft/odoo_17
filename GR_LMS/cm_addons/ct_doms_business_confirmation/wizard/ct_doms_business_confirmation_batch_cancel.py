...
class CtDomsBusConBatchCancel(models.TransientModel):
    _name = 'ct.doms.bus.con.batch.cancel'
    _description = "Batch Cancel"

    transaction_ids = fields.Many2many('ct.doms.business.confirmation', string="Batch Cancel", default=lambda self: self.env.context.get('active_ids'), c_rule=True)
