...
class CtDomsBusinessConfirmation(http.Controller):
    @validate_token
    @http.route('/ct_doms_business_confirmation/get_api', type='json', auth='none', methods=['GET','OPTIONS'], csrf=False, cors='*', save_session=False)
    def transaction_get_api(self, **kw):
        ...
        try:
            ...
            trans_rec = http.request.env['ct.transaction'].search([('id', '=', search_id)], limit=1)
    ...
    @validate_token
    @http.route('/ct_doms_business_confirmation/post_cancel_api', type='json', auth='none', methods=['POST', 'OPTIONS'], csrf=False, save_session=False)
    def transaction_cancel_api(self, **kw):
        try:
            json_data = json.loads(request.httprequest.data)
