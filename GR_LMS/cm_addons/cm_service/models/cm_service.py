...
class CmService(models.Model):
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_service where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Service name must be unique"))
	
    @api.onchange('bus_location')
    def onchange_bus_location(self):
        if self.bus_location == 'pan_india':
            self.ser_pro_company_id = self.env['res.company'].search([('code', '=', 'GMPL')], limit=1).id
        elif self.bus_location == 'exim':
            self.ser_pro_company_id = self.env['res.company'].search([('code', '=', 'GSCS')], limit=1).id
        else:
            self.ser_pro_company_id = False

    def validations(self):
        warning_msg = []
        if not self.line_ids:
            warning_msg.append("System not allow to approve with empty line charges details")
    ...
    def retrieve_dashboard(self):
        ...
        result['all_active'] = cm_service.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_service.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_service.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_service.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_service.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_service.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_service.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_service.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_service.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_service.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_service.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
