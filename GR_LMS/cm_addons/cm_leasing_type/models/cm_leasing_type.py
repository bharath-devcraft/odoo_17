...
CM_LEASING_TYPE = 'cm.leasing.type'
RES_USERS = 'res.users'
...
class CmLeasingType(models.Model):
    _name = 'cm.leasing.type'
    _description = 'Leasing Type'
    ...
    line_ids = fields.One2many('cm.leasing.type.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_leasing_type where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Leasing type name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_leasing_type where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Leasing type short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmLeasingType, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_leasing_type = self.env[CM_LEASING_TYPE]
        result['all_draft'] = cm_leasing_type.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_leasing_type.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_leasing_type.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_leasing_type.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_leasing_type.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_leasing_type.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_leasing_type.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_leasing_type.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_leasing_type.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_leasing_type.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_leasing_type.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_leasing_type.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
