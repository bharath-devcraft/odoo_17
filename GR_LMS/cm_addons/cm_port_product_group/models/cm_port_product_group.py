...
CM_PORT_PRODUCT_GROUP = 'cm.port.product.group'
RES_USERS = 'res.users'
...
class CmPortProductGroup(models.Model):
    _name = 'cm.port.product.group'
    _description = 'Port Product Group'
    ...
    line_ids = fields.One2many('cm.port.product.group.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_port_product_group where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Port Product Group name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmPortProductGroup, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_port_product_group = self.env[CM_PORT_PRODUCT_GROUP]
        result['all_draft'] = cm_port_product_group.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_port_product_group.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_port_product_group.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_port_product_group.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_port_product_group.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_port_product_group.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_port_product_group.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_port_product_group.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_port_product_group.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_port_product_group.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_port_product_group.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_port_product_group.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
