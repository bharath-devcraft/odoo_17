...
class CtNocostTransaction(http.Controller):
    @validate_token
    @http.route('/ct_item_issue/get_api', type='json', auth='none', methods=['GET','OPTIONS'], csrf=False, cors='*', save_session=False)
    def item_issue_get_api(self, **kw):
        ...
        try:
            ...
            trans_rec = http.request.env['ct.item.issue'].search([('id', '=', search_id)], limit=1)
    ...
    @validate_token
    @http.route('/ct_item_issue/post_cancel_api', type='json', auth='none', methods=['POST', 'OPTIONS'], csrf=False, save_session=False)
    def item_issue_cancel_api(self, **kw):
        try:
            json_data = json.loads(request.httprequest.data)
            ...
            tran_rec = http.request.env['ct.item.issue']
