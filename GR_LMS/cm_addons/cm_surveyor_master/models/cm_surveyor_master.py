# -*- coding: utf-8 -*-
import time
from odoo.addons.custom_properties.decorators import validation, is_special_char, valid_mobile_no, valid_phone_no, valid_email, valid_pin_code, valid_gst_no, valid_cin_no, valid_website, valid_tin_no, valid_tan_no, valid_cst_no,valid_aadhaar_no
...
CM_SURVEYOR_MASTER ='cm.surveyor.master'
CM_CITY = 'cm.city'
...
BUS_LOCATION = [('pan_india', 'PAN India'), ('exim', 'Exim(Global)')]
...
class CmSurveyorMaster(models.Model):
    _name = 'cm.surveyor.master'
    _description = 'Surveyor Master'
    ...
    parent_company_id = fields.Many2one(CM_SURVEYOR_MASTER, string="Parent Company", domain=[('status', '=', 'active'),('active_trans', '=', True)])
    ...
    is_registered = fields.Selection(selection=YES_OR_NO, string="Is Registered Surveyor", copy=False)
    ...
    per_inspection = fields.Boolean(string="Periodic inspection", copy=False)
    external = fields.Boolean(string="External", copy=False)
    cleaning = fields.Boolean(string="Cleaning", copy=False)
    condition = fields.Boolean(string="Condition", copy=False)
    on_hire = fields.Boolean(string="On Hire", copy=False)
    off_hire = fields.Boolean(string="Off Hire", copy=False)
    repair_stage_inspect = fields.Boolean(string="Repair & Stage Wise Inspection", copy=False, default=False)
    exp_years = fields.Integer(string="Industry Experience", copy=False)
    bus_location = fields.Selection(selection=BUS_LOCATION, string="Agreement Location", copy=False)
    ...
    line_ids = fields.One2many('cm.surveyor.master.line', 'header_id', string="Additional Contacts", copy=True, c_rule=True)
    line_ids_a = fields.One2many('cm.surveyor.master.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    line_ids_b = fields.One2many('cm.surveyor.master.bank.details.line', 'header_id', string="Bank Details", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_surveyor_master where upper(REPLACE(name, ' ', ''))  = '%s' 
            and id != %s and company_id = %s""" %(name, self.id,self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Surveyor master name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_surveyor_master where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Surveyor master short name must be unique"))
    ...
    def gst_no_validation(self):
        if self.gst_no:
            ...
            if self.entry_type != 'name_change':
                existing_gst = self.env[CM_SURVEYOR_MASTER].search_count([('gst_no', '=', self.gst_no), ('id', '!=', self.id), ('company_id', '=', self.company_id.id)])
                if existing_gst > 0:
    ...
    def validity_validations(self):
        if self.valid_from_date and self.valid_to_date and self.validity_range == 'limited':
    ...
    def write(self, vals):
        ...
        return super(CmSurveyorMaster, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_surveyor_master = self.env[CM_SURVEYOR_MASTER]
        result['all_draft'] = cm_surveyor_master.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_surveyor_master.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_surveyor_master.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_surveyor_master.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_surveyor_master.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_surveyor_master.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_surveyor_master.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_surveyor_master.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_surveyor_master.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_surveyor_master.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_surveyor_master.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_surveyor_master.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
