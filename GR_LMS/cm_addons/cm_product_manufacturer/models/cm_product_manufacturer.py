...
CM_PRODUCT_MANUFACTURER = 'cm.product.manufacturer'
RES_USERS = 'res.users'
...
class CmProductManufacturer(models.Model):
    _name = 'cm.product.manufacturer'
    ...
    contact_no = fields.Char(string="Contact No", size=15, copy=False)
    contact_no_2 = fields.Char(string="Contact No 2", size=15, copy=False)
    ...
    mb_2_cc_id = fields.Many2one(CM_COUNTRY_CODE, string="Mobile2 Country Code", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    ...
    line_ids = fields.One2many('cm.product.manufacturer.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_product_manufacturer where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Product Manufacturer name must be unique"))
    ...
    def onchange_country_id(self):
        if self.country_id:
            ...
            self.mb_2_cc_id = c_code
            
        else:
            self.country_code = False
            self.mb_cc_id = False
            self.mb_2_cc_id = False
    ...
    def write(self, vals):
        ...
        return super(CmProductManufacturer, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_product_manufacturer = self.env[CM_PRODUCT_MANUFACTURER]
        result['all_draft'] = cm_product_manufacturer.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_product_manufacturer.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_product_manufacturer.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_product_manufacturer.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_product_manufacturer.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_product_manufacturer.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_product_manufacturer.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_product_manufacturer.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_product_manufacturer.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_product_manufacturer.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_product_manufacturer.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_product_manufacturer.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
