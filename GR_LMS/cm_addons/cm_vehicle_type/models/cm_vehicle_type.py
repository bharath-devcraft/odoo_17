...
CM_VEHICLE_TYPE = 'cm.vehicle.type'
RES_USERS = 'res.users'
...
class CmVehicleType(models.Model):
    _name = 'cm.vehicle.type'
    ...
    line_ids = fields.One2many('cm.vehicle.type.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_vehicle_type where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Vehicle type name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmVehicleType, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_vehicle_type = self.env[CM_VEHICLE_TYPE]
        result['all_draft'] = cm_vehicle_type.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_vehicle_type.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_vehicle_type.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_vehicle_type.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_vehicle_type.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_vehicle_type.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_vehicle_type.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_vehicle_type.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_vehicle_type.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_vehicle_type.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_vehicle_type.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_vehicle_type.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
