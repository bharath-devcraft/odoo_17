...
class CmTransportLocation(models.Model):
    _name = 'cm.transport.location'
    _description = 'Transport Location'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'name asc'


    name = fields.Char(string="Location Name", index=True, copy=False)
    ...
    city_id = fields.Many2one('cm.city', string="City", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    ...
    line_ids = fields.One2many('cm.transport.location.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_transport_location where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Transport location name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_transport_location where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Transport location short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmTransportLocation, self).write(vals)
    
    @api.onchange('city_id')
    def onchange_city_id(self):
        if self.city_id:
            self.state_id = self.city_id.state_id.id if self.city_id.state_id else False,
            self.country_id = self.city_id.country_id.id if self.city_id.country_id else False
    ...
    def retrieve_dashboard(self):
        ...
        cm_transport_location = self.env[CM_TRANSPORT_LOCATION]
        result['all_draft'] = cm_transport_location.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_transport_location.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_transport_location.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_transport_location.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_transport_location.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_transport_location.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_transport_location.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_transport_location.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_transport_location.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_transport_location.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_transport_location.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_transport_location.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
