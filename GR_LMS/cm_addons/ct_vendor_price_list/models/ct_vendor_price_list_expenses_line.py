...
class CtVendorPriceListExpensesLine(models.Model):
    _name = 'ct.vendor.price.list.expenses.line'
    _description = 'Other Charges'
    _order = 'id asc'

    header_id = fields.Many2one('ct.vendor.price.list', string="Header Ref", index=True, required=True, ondelete='cascade', c_rule=True)
    ...
    def _convert_to_tax_base_line_dict(self):
        ...
        return self.env[ACCOUNT_TAX]._convert_to_tax_base_line_dict(
            self,
            partner=self.header_id.user_id, # vendor_id
