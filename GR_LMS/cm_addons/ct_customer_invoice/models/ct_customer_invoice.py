...
CT_CUSTOMER_INVOICE = 'ct.customer.invoice'
RES_USERS = 'res.users'
...
class CtCustomerInvoice(models.Model):
    _name = 'ct.customer.invoice'
    _description = 'Customer Invoice'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'entry_date desc,name desc'

    name = fields.Char(string="Invoice No", readonly=True, index=True, copy=False, size=30, c_rule=True)
    status = fields.Selection(selection=CUSTOM_STATUS, string="Status", copy=False, default="draft", readonly=True, store=True, tracking=True)
    entry_date = fields.Date(string="Invoice Date", copy=False, default=fields.Date.today)
    bc_id = fields.Many2one('ct.business.confirmation', string="BC No", ondelete='restrict', domain=[('status', '=', 'approved'),('active_trans', '=', True),('invoice_status', '=', 'pending')])
    ...
    dc_no = fields.Char(string="DC No", copy=False)
    ...
    model_ref_no = fields.Char(string="Ref No")
    ...
    line_ids_a = fields.One2many('ct.customer.invoice.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    line_ids_c = fields.One2many('ct.customer.invoice.acc.line', 'header_id', string="Flexi Details", copy=True, c_rule=True)
    line_ids_e = fields.One2many('ct.customer.invoice.pri.details.line', 'header_id', string="Charges Details", copy=True, c_rule=True)
    line_ids_f = fields.One2many('ct.customer.invoice.terms.conditions.line', 'header_id', string="Terms and Conditions", copy=True, c_rule=True)
    line_ids_g = fields.One2many('ct.customer.invoice.tax.line', 'header_id', string="Tax Breakup", copy=True, c_rule=True)
    line_ids_h = fields.One2many('ct.customer.invoice.receipt.details.line', 'header_id', string="Receipt Details", copy=True, c_rule=True)
    ...
    def onchange_bc_id(self):
        ...
        self.payment_mode = self.bc_id.payment_mode
        ...
        self.po_no = self.bc_id.po_no
        self.po_date = self.bc_id.po_date
        self.round_off_amt = self.bc_id.round_off_amt
        self.net_amt = self.bc_id.net_amt
        self.manual_round_off = self.bc_id.manual_round_off
        self.combined_codes = self.bc_id.combined_codes
        self.line_ids_e = price_rec
        self.line_ids_f = [(0, 0, line.copy_data()[0]) for line in self.bc_id.line_ids_f]
        ...
        if 'FLOS' in self.combined_codes:
            self.operational_type = self.bc_id.operational_type
            self.flexi_stuff_qty = self.bc_id.flexi_stuff_qty
            self.stuff_date = self.bc_id.stuff_date
            self.stuff_address = self.bc_id.stuff_address
        ...
        if self.status == 'wfa':
            ...
            if not self.name:
                sequence_id = self.env[IR_SEQUENCE].search(
                        [('code', '=', CT_CUSTOMER_INVOICE)], limit=1)
                if sequence_id:
                    self.env.cr.execute(
            ...
            self.bc_id.invoiced_amt += self.net_amt

            if any(code in self.combined_codes for code in ('FLBS', 'FLAS')):
                self.env['ct.delivery.challan'].auto_delivery_challan_entry_creation(cus_inv_rec=self)
            ...
            self.flexi_customer_invoice_mail_data_design(
                trans_rec = self,
                mail_queue_name = 'Flexi Customer Invoice Approve Mail',
                subject = f"#customer-invoice-approved# {self.service_id.name} - {self.name}",
                mail_config_name = 'Flexi Customer Invoice Approve Mail'
            )
    ...
    def write(self, vals):
        ...
        return super(CtCustomerInvoice, self).write(vals)

    def auto_customer_invoice_entry_creation(self, **kw):
        issue_rec = kw.get('issue_rec', '')

        if issue_rec and any(code in issue_rec.combined_codes for code in ('FLBS', 'FLAS')):
            price_rec = [(0, 0, {
                    'chrg_head_id': rec.chrg_head_id.id,
                    'description': rec.description,
                    'uom_id': rec.uom_id.id,
                    ...
                    'tot_amt': rec.tot_amt,
                    'line_tot_amt': rec.line_tot_amt,
                }) for rec in issue_rec.bc_id.line_ids_e]

            invoice_model = issue_rec.env['ct.customer.invoice']
            invoice_rec = invoice_model.create({
                'bc_id': issue_rec.bc_id.id,
                'service_id': issue_rec.service_id.id,
                'payment_mode': issue_rec.bc_id.payment_mode,
                'customer_id': issue_rec.bc_id.customer_id.id,
                'cus_bill_address': issue_rec.bc_id.cus_bill_address,
                'cus_del_address': issue_rec.bc_id.cus_del_address,
                'gst_no': issue_rec.bc_id.customer_id.gst_no,
                'pan_no': issue_rec.bc_id.customer_id.pan_no,
                'contact_person': issue_rec.bc_id.contact_person,
                'mobile_no': issue_rec.bc_id.mobile_no,
                'email': issue_rec.bc_id.email,
                'po_no': issue_rec.bc_id.po_no,
                'po_date': issue_rec.bc_id.po_date,
                'line_ids_e': price_rec,
                'line_ids_f': [(0, 0, line.copy_data()[0]) for line in issue_rec.bc_id.line_ids_f],
                'line_ids_a': [(0, 0, line.copy_data()[0]) for line in issue_rec.line_ids_a],
                'round_off_amt': issue_rec.bc_id.round_off_amt,
                'net_amt': issue_rec.bc_id.net_amt,
                'manual_round_off': issue_rec.bc_id.manual_round_off,
                'combined_codes': issue_rec.combined_codes,
                'model_ref_no': issue_rec.name,
                'entry_mode': 'auto'
            })

            if invoice_rec and 'FLBS' in issue_rec.combined_codes:
                invoice_rec.write({
                    'vendor_id': issue_rec.vendor_id.id,
                    'accessories': 'with_accessories' if issue_rec.accessories_req == 'yes' else 'without_accessories',
                })

            invoice_rec.line_ids_c = [(0, 0, {
                'flexi_type': rec.flexi_type,
                'flexi_layer_type_id': rec.flexi_layer_type_id.id if rec.flexi_layer_type_id else False,
                'flexi_capacity_id': rec.flexi_capacity_id.id,
                'vendor_id': rec.vendor_id.id if rec.vendor_id else False,
            ...
            }) for rec in issue_rec.line_ids_c]

            invoice_rec.onchange_currency_id()

    def  get_default_mail_ids(self, **kw):
        mail_ids = {}
        trans_rec = self.env[CT_CUSTOMER_INVOICE].search([('id', '=', kw.get('trans_id', False))])
    ...
    def flexi_customer_invoice_mail_data_design(self, **kw):
        self.env.cr.execute(
                "SELECT ctm_flexi_customer_invoice_approve_mail(%s, %s, %s, %s, %s)",
        ...
        if trans_rec and data[0][0] and mail_queue_name and subject and mail_config_name:
            ...
            vals = self.env['cp.mail.configuration'].mail_config_mailids_data(
                mail_type=mail_type, model_name=CT_CUSTOMER_INVOICE, mail_name=mail_config_name)
    ...
    def retrieve_dashboard(self):
        ...
        ct_trans = self.env[CT_CUSTOMER_INVOICE]
