...
class CtGatePassLine(models.Model):
    _name = 'ct.gate.pass.line'
    _description = 'Details'
    _order = 'description asc'

    header_id = fields.Many2one('ct.gate.pass', string="Header Ref", index=True, required=True, ondelete='cascade', c_rule=True)

    product_id = fields.Many2one('product.template', string="Product Name", index=True, ondelete='restrict')
    ...
    line_ids = fields.One2many('ct.gate.pass.serialno.line', 'header_id', string='Serial Number', copy=True, c_rule=True)
