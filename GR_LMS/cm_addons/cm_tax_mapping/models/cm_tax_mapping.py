...
class CmTaxMapping(models.Model):
    _name = 'cm.tax.mapping'
    ...
    hs_id = fields.Many2one('cm.hsn.code', string="SAC Code", copy=False, domain=[('status', '=', 'active'),('active_trans', '=', True)])
    cgst_id = fields.Many2one(ACCOUNT_TAX, string="CGST", check_company=True, copy=False, ondelete='restrict', domain="[('status', '=', 'active'),('active_trans', '=', True),('tax_group_id.name', '=', 'CGST')]")
    sgst_id = fields.Many2one(ACCOUNT_TAX, string="SGST", check_company=True, copy=False, ondelete='restrict', domain="[('status', '=', 'active'),('active_trans', '=', True),('tax_group_id.name', '=', 'SGST')]")
    igst_id = fields.Many2one(ACCOUNT_TAX, string="IGST", check_company=True, copy=False, ondelete='restrict', domain="[('status', '=', 'active'),('active_trans', '=', True),('tax_group_id.name', '=', 'IGST')]")
    ...
    line_ids_a = fields.One2many('cm.tax.mapping.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_tax_mapping where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Tax Mapping name must be unique"))
        
    @api.onchange('charges_id')
    def onchange_charges_id(self):
        if self.charges_id:
            self.name = self.charges_id.name
        else:
            self.name = False
    ...
    def onchange_hs_id(self):
        if self.hs_id:
            self.cgst_id = self.hs_id.cgst_id
            self.sgst_id = self.hs_id.sgst_id
            self.igst_id = self.hs_id.igst_id
        else:
            self.cgst_id = False
            self.sgst_id = False
            self.igst_id = False
    ...
    def write(self, vals):
        ...
        return super(CmTaxMapping, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_tax_mapping = self.env[CM_TAX_MAPPING]
        result['all_draft'] = cm_tax_mapping.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_tax_mapping.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_tax_mapping.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_tax_mapping.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_tax_mapping.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_tax_mapping.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_tax_mapping.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_tax_mapping.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_tax_mapping.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_tax_mapping.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_tax_mapping.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_tax_mapping.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
