...
CM_CHARGES_HEADS = 'cm.charges.heads'
RES_USERS = 'res.users'
...
class CmChargesHeads(models.Model):
    ...
    def _get_dynamic_domain(self):
        uom_rec = self.env['uom.uom'].search([('name', '=', 'Units'),('status', '=', 'active'),('active_trans', '=', True)], limit=1)
        return uom_rec.id if uom_rec else None
    ...
    line_ids = fields.One2many('cm.charges.heads.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_charges_heads where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Charges heads name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_charges_heads where upper(REPLACE(short_name, ' ', ''))  = '%s'
    ...
    def write(self, vals):
        ...
        return super(CmChargesHeads, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_active'] = cm_charges_heads.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_charges_heads.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_charges_heads.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_charges_heads.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_charges_heads.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_charges_heads.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_charges_heads.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_charges_heads.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_charges_heads.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_charges_heads.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_charges_heads.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
