...
class CmBlClause(models.Model):
    _name = 'cm.bl.clause'
    _description = 'BL Clause'
    ...
    line_ids = fields.One2many('cm.bl.clause.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_bl_clause where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Bl Clause description must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmBlClause, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_bl_clause = self.env[CM_BL_CLAUSE]
        result['all_draft'] = cm_bl_clause.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_bl_clause.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_bl_clause.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_bl_clause.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_bl_clause.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_bl_clause.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_bl_clause.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_bl_clause.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_bl_clause.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_bl_clause.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_bl_clause.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_bl_clause.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
