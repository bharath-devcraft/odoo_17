...
BUSINESS_LOCATION =  [('pan_india','PAN India'),
			   ('exim', 'Exim(Global)')]

class CmTankLeaseTariff(models.Model):
	_name = 'cm.tank.lease.tariff'
	_description = 'Tank Lease Tariff'
	...
	def onchange_business_location(self):
		if self.business_location == 'pan_india':
			self.currency_id = self.env.ref('base.INR').id
			self.name = 'PAN India'
		elif self.business_location == 'exim':
			self.currency_id = self.env.ref('base.USD').id
			self.name = 'Exim(Global)'
		else:
			pass
	...
	def write(self, vals):
		...
		return super(CmTankLeaseTariff, self).write(vals)
