...
class CmCustomer(models.Model):
	...
	line_ids_b = fields.One2many('cm.customer.billing.address.line', 'header_id', string="Billing Address", copy=True, c_rule=True)
	line_ids_c = fields.One2many('cm.customer.bank.details.line', 'header_id', string="Bank Details", copy=True, c_rule=True)
	line_ids_d = fields.One2many('cm.customer.delivery.address.line', 'header_id', string="Delivery Address", copy=True, c_rule=True)
	line_ids_e = fields.One2many('cm.customer.history.line', 'header_id', string="History", copy=True, c_rule=True)
	...
	def short_name_validation(self):
		if self.short_name:
			if is_special_char(self.env,self.short_name):
				raise UserError(_("Special character is not allowed in customer ID field"))
			...
			if self.env.cr.fetchone():
				raise UserError(_("Customer ID must be unique"))
	...
	def entry_approve(self):
		if self.status in ('draft', 'editable'):			
			...
			if not self.short_name:
				...
				self.short_name = self.name[:4].upper() + sequence			
