...
CM_CARRIER_PRODUCT_APPROVAL = 'cm.carrier.product.approval'
RES_USERS = 'res.users'
...
class CmCarrierProductApproval(models.Model):
    _name = 'cm.carrier.product.approval'
    _description = 'Carrier Product Approval'
    ...
    ap_number = fields.Char(string="Approval Number")
    ...
    line_ids_a = fields.One2many('cm.carrier.product.approval.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def write(self, vals):
        ...
        return super(CmCarrierProductApproval, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_carrier_product_approval = self.env[CM_CARRIER_PRODUCT_APPROVAL]
        result['all_draft'] = cm_carrier_product_approval.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_carrier_product_approval.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_carrier_product_approval.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_carrier_product_approval.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_carrier_product_approval.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_carrier_product_approval.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_carrier_product_approval.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_carrier_product_approval.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_carrier_product_approval.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_carrier_product_approval.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_carrier_product_approval.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_carrier_product_approval.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
