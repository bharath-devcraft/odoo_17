...
class CmCarrierProductApprovalAttachmentLine(models.Model):
    _name = 'cm.carrier.product.approval.attachment.line'
    _description = 'Attachments'
    _order = 'attach_date desc'

    header_id = fields.Many2one('cm.carrier.product.approval', string="Header Ref",index=True, required=True, ondelete='cascade', c_rule=True)
