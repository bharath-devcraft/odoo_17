...
class CtRfq(http.Controller):
    @validate_token
    @http.route('/ct_rfq/get_api', type='json', auth='none', methods=['GET','OPTIONS'], csrf=False, cors='*', save_session=False)
    def rfq_get_api(self, **kw):
        ...
        try:
            ...
            trans_rec = http.request.env['ct.rfq'].search([('id', '=', search_id)], limit=1)
    ...
    @validate_token
    @http.route('/ct_rfq/post_cancel_api', type='json', auth='none', methods=['POST', 'OPTIONS'], csrf=False, save_session=False)
    def rfq_cancel_api(self, **kw):
        try:
            json_data = json.loads(request.httprequest.data)
            ...
            tran_rec = http.request.env['ct.rfq']
