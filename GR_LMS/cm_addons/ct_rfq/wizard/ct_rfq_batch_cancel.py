...
class CtRfqBatchCancel(models.TransientModel):
    _name = 'ct.rfq.batch.cancel'
    _description = "Batch Cancel"

    transaction_ids = fields.Many2many('ct.rfq', string="Batch Cancel", default=lambda self: self.env.context.get('active_ids'), c_rule=True)
    ...
    def _compute_dynamic_value(self):
        if self.env.context.get('active_ids'):
            invalid_ids = [tran_rec.name if tran_rec.name else tran_rec.draft_name for rec in self.env.context.get('active_ids') for tran_rec in self.env['ct.rfq'].browse(rec) if tran_rec.status not in ('approved','draft')]
            if invalid_ids:
