...
class CtGRNScheduler(models.Model):
    _name = 'ct.grn.scheduler'
    _description = 'GRN Scheduler'

    def custom_scheduler_mail(self):
        mail_name = "Custom Scheduler Mail"

        approved_count = self.env['ct.grn'].search_count([('status', '=', 'approved'),('ap_rej_date', '>=', fields.Date.today())])
