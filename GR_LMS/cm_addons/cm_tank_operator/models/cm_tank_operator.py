...
class CmTankOperator(models.Model):
    _name = 'cm.tank.operator'
    ...
    def default_get(self, vals):
        ...
        kyc_recs = self.env['cm.kyc.master'].search([
            ('model_id', '=', model_id),
            ('status', '=', 'active'),
            ('active_trans', '=', True)
        ], limit=1)
        if kyc_recs and kyc_recs.line_ids:
            res['line_ids_a'] = [
                (0, 0, {
                    'attach_desc': rec.name,
                    'mandatory': rec.mandatory,
                    'validity': rec.validity,
                    'validity_period': rec.validity_period,
                    'attach_user_id': self.env.user.id,
                    'attach_date': time.strftime('%Y-%m-%d %H:%M:%S'),
                    'entry_mode': 'auto'
                })
                for rec in kyc_recs.line_ids
            ]
    ...
    line_ids_b = fields.One2many('cm.tank.operator.bank.details.line', 'header_id', string="Bank Details", copy=True, c_rule=True)
    line_ids_c = fields.One2many('cm.tank.operator.intermediatory.bank.line', 'header_id', string="Intermediatory Bank", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_tank_operator where upper(REPLACE(name, ' ', ''))  = '%s' 
    ...
    def gst_no_validation(self):
        if self.gst_no:
            ...
            if self.entry_type == 'new':
                existing_gst = self.env[CM_TANK_OPERATOR].search_count([('gst_no', '=', self.gst_no), ('id', '!=', self.id), ('company_id', '=', self.company_id.id)])
                if existing_gst > 0:
                    raise UserError(_("GST number must be unique for new entry type"))

    @api.constrains('cin_no')
    def cin_no_validation(self):
        if self.cin_no:
            if self.cin_no and is_special_char(self.env,self.cin_no):
                raise UserError(_("Special character is not allowed in CIN number field"))
            if ' ' in self.cin_no:
                raise UserError(_("Space is not allowed in CIN number field"))
    ...
    def pan_no_validation(self):
        if self.pan_no:
            ...
            existing_record = self.env[CM_TANK_OPERATOR].search_count([('pan_no', '=', self.pan_no),('id', '!=', self.id), ('company_id', '=', self.company_id.id)])
            if existing_record:
    ...
    def append_warnings(self, warning_msg, issue_date_attachments, 
        ...
        if file_required_attachments:
            warning_msg.append(f"File is required in attachments tab. Ref: {', '.join(file_required_attachments)}")
        if issue_date_required_attachments:
            warning_msg.append(f"Issue date is required in attachments tab. Ref: {', '.join(issue_date_required_attachments)}")

    def validate_attachment_lines(self, warning_msg):
        today = fields.Date.today()
        issue_date_attachments = []
        validity_period_attachments = []
        file_required_attachments = []
        issue_date_required_attachments = []
        for attach_line in self.line_ids_a:
            if attach_line.from_date and attach_line.from_date < today:
                issue_date_attachments.append(attach_line.attach_desc)
            if attach_line.validity_period and attach_line.validity_period < 0:
                validity_period_attachments.append(attach_line.attach_desc)
            if attach_line.entry_mode == 'auto' and attach_line.mandatory == 'yes' and not attach_line.attachment_ids:
                file_required_attachments.append(attach_line.attach_desc)
            if attach_line.entry_mode == 'auto'and  attach_line.validity == 'limited' and not attach_line.from_date:
                issue_date_required_attachments.append(attach_line.attach_desc)
        self.append_warnings(warning_msg, issue_date_attachments, 
                            validity_period_attachments, 
                            file_required_attachments, 
                            issue_date_required_attachments)
    ...
    def validations(self):
        ...
        self.validate_attachment_lines(warning_msg)
        if warning_msg:
            formatted_messages = "\n".join(warning_msg)
    ...
    def write(self, vals):
        ...
        return super(CmTankOperator, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_inactive'] = cm_tank_operator.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_tank_operator.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_tank_operator.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_tank_operator.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_tank_operator.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_tank_operator.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_tank_operator.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_tank_operator.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_tank_operator.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_tank_operator.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
