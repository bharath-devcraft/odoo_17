...
CM_AGENT_RESTRICTION = 'cm.agent.restriction'
RES_USERS = 'res.users'
...
class CmAgentRestriction(models.Model):
    _name = 'cm.agent.restriction'
    _description = 'Agent Restriction'
    ...
    port_id = fields.Many2one('cm.port', string="POD", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
    ...
    line_ids_a = fields.One2many('cm.agent.restriction.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_agent_restriction where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Agent Restriction name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmAgentRestriction, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_agent_restriction = self.env[CM_AGENT_RESTRICTION]
        result['all_draft'] = cm_agent_restriction.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_agent_restriction.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_agent_restriction.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_agent_restriction.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_agent_restriction.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_agent_restriction.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_agent_restriction.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_agent_restriction.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_agent_restriction.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_agent_restriction.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_agent_restriction.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_agent_restriction.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
