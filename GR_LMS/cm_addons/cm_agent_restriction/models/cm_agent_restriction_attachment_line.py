...
class CmAgentRestrictionAttachmentLine(models.Model):
    _name = 'cm.agent.restriction.attachment.line'
    _description = 'Attachments'
    _order = 'attach_date desc'

    header_id = fields.Many2one('cm.agent.restriction', string="Header Ref",index=True, required=True, ondelete='cascade', c_rule=True)
