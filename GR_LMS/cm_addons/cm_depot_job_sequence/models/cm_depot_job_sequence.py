...
CM_DEPOT_JOB_SEQUENCE = 'cm.depot.job.sequence'
RES_USERS = 'res.users'
...
class CmDepotJobSequence(models.Model):
    _name = 'cm.depot.job.sequence'
    _description = 'Depot Job Sequence'
    ...
    line_ids = fields.One2many('cm.depot.job.sequence.line', 'header_id', string="Job Sequence Details", copy=True, c_rule=True)
    line_ids_a = fields.One2many('cm.depot.job.sequence.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_depot_job_sequence where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Depot Job Sequence name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmDepotJobSequence, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_depot_job_sequence = self.env[CM_DEPOT_JOB_SEQUENCE]
        result['all_draft'] = cm_depot_job_sequence.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_depot_job_sequence.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_depot_job_sequence.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_depot_job_sequence.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_depot_job_sequence.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_depot_job_sequence.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_depot_job_sequence.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_depot_job_sequence.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_depot_job_sequence.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_depot_job_sequence.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_depot_job_sequence.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_depot_job_sequence.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
