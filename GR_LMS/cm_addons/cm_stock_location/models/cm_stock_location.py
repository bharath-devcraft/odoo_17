...
CM_STOCK_LOCATION = 'cm.stock.location'
RES_USERS = 'res.users'
...
USAGE_LOCATION = [('internal', 'Internal Location'), ('vendor', 'Vendor Location')]
LOCATION_TYPE = [('main', 'Main'), ('sub', 'Sub')]

class CmStockLocation(models.Model):
    _name = 'cm.stock.location'
    _description = 'Stock Location'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'avatar.mixin']
    _order = 'name asc'


    name = fields.Char(string="Stock Location", index=True, copy=False)
    ...
    usage_location = fields.Selection(selection=USAGE_LOCATION, string="Usage Location")
    location_type = fields.Selection(selection=LOCATION_TYPE, string="Location Type")
    ...
    line_ids_a = fields.One2many('cm.stock.location.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_stock_location where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Stock Location must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_stock_location where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Stock Location short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmStockLocation, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_stock_location = self.env[CM_STOCK_LOCATION]
        result['all_draft'] = cm_stock_location.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_stock_location.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_stock_location.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_stock_location.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_stock_location.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_stock_location.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_stock_location.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_stock_location.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_stock_location.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_stock_location.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_stock_location.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_stock_location.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
