...
CM_NATURE_OF_JOB = 'cm.nature.of.job'
RES_USERS = 'res.users'
...
CUSTOM_STATUS = [
        ...
        ('active', 'Active'),
        ('inactive', 'Inactive')]
PHOTOS_REQUIRED=[('required','Required'),
               ('not_required', 'Not Required')]

ENTRY_MODE =  [('manual','Manual'),
               ('auto', 'Auto')]

class CmNatureOfJob(models.Model):
    _name = 'cm.nature.of.job'
    _description = 'Nature Of Job'
    ...
    photos_required = fields.Selection(selection=PHOTOS_REQUIRED, string="Photos Required")
    ...
    line_ids_a = fields.One2many('cm.nature.of.job.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def name_validation(self):
        if self.name:
            ...
            from cm_nature_of_job where upper(REPLACE(name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Nature Of Job name must be unique"))
    ...
    def short_name_validation(self):
        if self.short_name:
            ...
            from cm_nature_of_job where upper(REPLACE(short_name, ' ', ''))  = '%s'
            and id != %s and company_id = %s""" %(short_name, self.id, self.company_id.id))
            if self.env.cr.fetchone():
                raise UserError(_("Nature Of Job short name must be unique"))
    ...
    def write(self, vals):
        ...
        return super(CmNatureOfJob, self).write(vals)
     
    @api.model
    def retrieve_dashboard(self):
        result = {}
        
        cm_nature_of_job = self.env[CM_NATURE_OF_JOB]
        result['all_draft'] = cm_nature_of_job.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_nature_of_job.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_nature_of_job.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_nature_of_job.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_nature_of_job.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_nature_of_job.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_nature_of_job.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_nature_of_job.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_nature_of_job.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_nature_of_job.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_nature_of_job.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_nature_of_job.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
