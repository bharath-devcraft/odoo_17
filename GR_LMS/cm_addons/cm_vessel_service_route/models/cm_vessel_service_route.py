...
class CmVesselServiceRoute(models.Model):
    ...
    line_ids = fields.One2many('cm.vessel.service.route.line', 'header_id', string="Route Details", copy=True, c_rule=True)
    line_ids_a = fields.One2many('cm.vessel.service.route.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    line_ids_b = fields.One2many('cm.vessel.service.route.additional.checklist.line', 'header_id', string="Additional Checklist", copy=True, c_rule=True)
    ...
    def validate_detail_lines(self, warning_msg):
        ...
        if seq_duplicates:
            warning_msg.append(f"Duplicate sequence are not allowed. Ref: {', '.join(map(str, seq_duplicates))}")
    ...
    def validations(self):
        ...
        self.validate_detail_lines(warning_msg)
        if warning_msg:
    ...
    def write(self, vals):
        ...
        return super(CmVesselServiceRoute, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        result['all_active'] = cm_vessel_service_route.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_vessel_service_route.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_vessel_service_route.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_vessel_service_route.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_vessel_service_route.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_vessel_service_route.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_vessel_service_route.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_vessel_service_route.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_vessel_service_route.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_vessel_service_route.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_vessel_service_route.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
