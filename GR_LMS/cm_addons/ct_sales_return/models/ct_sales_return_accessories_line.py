...
class CtSalesReturnAccessoriesLine(models.Model):
    _name = 'ct.sales.return.acc.line'
    _description = 'Flexi Details'
    _order = 'id asc'

    header_id = fields.Many2one('ct.sales.return', string="Header Ref", index=True, required=True, ondelete='cascade', c_rule=True)
    ...
    return_lot_ids = fields.Many2many('ct.stock.lot', 'sales_return_lot_id', 'return_id', 'lot_id', string="Return Serial No", domain="[('product_id','=',flexi_bag_id)]")
    dc_lot_ids = fields.Many2many('ct.stock.lot', 'dc_sales_return_lot_id', 'dc_id', 'lot_id', string="DC Serial No", domain="[('product_id','=',flexi_bag_id)]")
    ...
    line_ids = fields.One2many('ct.sales.return.acc.details.line', 'header_id', string="Details", copy=True, c_rule=True)
