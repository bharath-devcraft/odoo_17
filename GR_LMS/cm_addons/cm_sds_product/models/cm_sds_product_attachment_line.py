...
class CmSdsProductAttachmentLine(models.Model):
    _name = 'cm.sds.product.attachment.line'
    _description = 'Attachments'
    _order = 'attach_date desc'

    header_id = fields.Many2one('cm.sds.product', string="Header Ref",index=True, required=True, ondelete='cascade', c_rule=True)
        ...
        for record in self:
            for attachment in record.attachment_ids:
                if attachment.mimetype != 'application/pdf':
                    raise ValidationError("Only PDF files are allowed in attachments tab")
