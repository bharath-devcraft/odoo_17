...
class CtStoreIssueRequestAccessoriesDetailsLine(models.Model):
    _name = 'ct.store.issue.request.acc.details.line'
    _description = 'Details'
    _order = 'id asc'

    header_id = fields.Many2one('ct.store.issue.request.acc.line', string="Header Ref", index=True, required=True, ondelete='cascade', c_rule=True)
