...
class CtStoreIssueRequestAccessoriesLine(models.Model):
    _name = 'ct.store.issue.request.acc.line'
    _description = 'Flexi Details'
    _order = 'id asc'

    header_id = fields.Many2one('ct.store.issue.request', string="Header Ref", index=True, required=True, ondelete='cascade', c_rule=True)
    ...
    line_ids = fields.One2many('ct.store.issue.request.acc.details.line', 'header_id', string="Details", copy=True, c_rule=True)
