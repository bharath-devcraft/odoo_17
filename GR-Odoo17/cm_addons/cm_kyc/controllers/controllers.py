...
class CmKyc(http.Controller):
    ...
    def disable_popup_notification(self, **kw):
        ...
        source = request.env['cm.kyc'].search([('model_id.model','=',model),('country_id','=', company_id.country_id.id)], limit=1, order="id desc")
        return True if fieldName in [field.name for field in source.field_ids] else False
