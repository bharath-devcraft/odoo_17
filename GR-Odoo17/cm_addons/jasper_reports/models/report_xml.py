...
class ReportXml(models.Model):
    ...
    def render_jasper(self, docids, data):
        ...
        param=self.env[record.get('model')].browse(docids)
        l="get_val"
        
        val=getattr(param,self.report_name)
        
        print("param********88", self.report_name)
        print("param********88...... val", val)
        print("param********88..............!!!! val()", val())
        
        data.update({'env': self.env, 'model': record.get('model'), 'parameters': json.dumps(val())})
        print("*****************************************", docids, data)
