...
TERMINAL_TYPE_OPTIONS = [('container_terminal','Container Terminal'),
                         ('break_bulk_terminals','Break-Bulk Terminals'),
                         ('neo_bulk_terminals', 'Neo-Bulk Terminals'),
                         ('oil_terminal', 'Oil Terminal')]

class CmTerminal(models.Model):
    _name = 'cm.terminal'
    _description = 'Cm Terminal'
    ...
    terminal_type = fields.Selection(selection=TERMINAL_TYPE_OPTIONS, string="Terminal Type", copy=False, readonly=True, store=True, tracking=True)
    icd = fields.Char(string="Inland Container Depot(ICD)", index=True, copy=False)
    ...
    line_ids = fields.One2many('cm.terminal.attachment.line', 'header_id', string="Attachments", copy=True, c_rule=True)
    ...
    def write(self, vals):
        ...
        return super(CmTerminal, self).write(vals)
    ...
    def retrieve_dashboard(self):
        ...
        cm_terminal = self.env[CM_TERMINAL]
        result['all_draft'] = cm_terminal.search_count([('status', '=', 'draft')])
        result['all_active'] = cm_terminal.search_count([('status', '=', 'active')])
        result['all_inactive'] = cm_terminal.search_count([('status', '=', 'inactive')])
        result['all_editable'] = cm_terminal.search_count([('status', '=', 'editable')])
        result['my_draft'] = cm_terminal.search_count([('status', '=', 'draft'), ('user_id', '=', self.env.uid)])
        result['my_active'] = cm_terminal.search_count([('status', '=', 'active'), ('user_id', '=', self.env.uid)])
        result['my_inactive'] = cm_terminal.search_count([('status', '=', 'inactive'), ('user_id', '=', self.env.uid)])
        result['my_editable'] = cm_terminal.search_count([('status', '=', 'editable'), ('user_id', '=', self.env.uid)])
              
        result['all_today_count'] = cm_terminal.search_count([('crt_date', '>=', fields.Date.today())])
        result['all_month_count'] = cm_terminal.search_count([('crt_date', '>=', datetime.today().replace(day=1))])
        result['my_today_count'] = cm_terminal.search_count([('user_id', '=', self.env.uid),('crt_date', '>=', fields.Date.today())])
        result['my_month_count'] = cm_terminal.search_count([('user_id', '=', self.env.uid), ('crt_date', '>=',datetime.today().replace(day=1))])
