...
import time
import re
from odoo.addons.custom_properties.decorators import validation, is_special_char, valid_mobile_no, valid_phone_no, valid_email, valid_pin_code, valid_gst_no
...
class CmAgent(models.Model):
	...
	short_name = fields.Char(string="Short Name", copy=False, size=50)
	...
	gst_no = fields.Char(string="GSTN", copy=False, size=15)
	
	### New Fields Added
	entry_type = fields.Selection(selection=ENTRY_TYPE, string="Entry Type", copy=False, default="new", tracking=True)
	is_registered_agent = fields.Selection(selection=YES_OR_NO, string="Is Registered Agent", copy=False)
	...
	agent_category = fields.Char(string="Agent Category", size=50)	
	industry_experience = fields.Char(string="Industry Experience", size=15)
	providing_services = fields.Selection(selection=PROVIDING_SERVICES, string="Providing Services", copy=False)
	contractual_agreements = fields.Selection(selection=YES_OR_NO, string="Contractual Agreements", copy=False)
	validity_range = fields.Selection(selection=VALIDITY_RANGE, string="Validity Range", copy=False)	
	from_date = fields.Date(string="From Date", copy=False, default=fields.Date.today)
	to_date = fields.Date(string="To Date", copy=False, default=fields.Date.today)	
	currency = fields.Char(string="Currency", size=10)
	servicing_ports_id = fields.Many2one(CM_MASTER, string="Servicing Ports", ondelete='restrict', domain=[('status', '=', 'active'),('active_trans', '=', True)])
	...
	line_ids = fields.One2many('cm.agent.line', 'header_id', string="Additional Contact Details", copy=True, c_rule=True)    
	...
	def short_name_validation(self):
		if self.short_name:
			...
			if self.env.cr.fetchone():
				raise UserError(_("Agent short name must be unique"))
	...
	def entry_approve(self):
		if self.status in ('draft', 'editable'):
			...
			if not self.short_name:				
				first_five_chars = self.name.replace(" ", "")[:4]
				customer_id = first_five_chars
			else:
