-- FUNCTION: public.ctm_template(integer, character, character, character)

-- DROP FUNCTION IF EXISTS public.ctm_template(integer, character, character, character);

CREATE OR REPLACE FUNCTION public.ctm_template(
	v_trans_id integer,
...
v_draft_date char varying (10000);
...
v_sl_no =1;
			...
			* {-webkit-font-smoothing: antialiased;}
			...
			select trans.draft_name,trans.draft_date,trans.name,to_char(trans.entry_date,'dd/mm/yyyy') as entry_date,div.name as Division,
			dep.name as department,supplier.name,partner.name,confirm_name.name,ap_name.name,trans.net_amt from  ct_transaction trans
			...
			LOOP                 
		  		FETCH cursor_1 INTO v_draft_name,v_draft_date,v_trans_name,v_entry_date,v_division,v_department,v_supplier,v_created_by,v_confirmed_by,v_approved_by,v_net_amt;
	   ...
	   if (v_trans_state != 'approved') then
			v_data=v_data || '<tr>
			<td colspan="3">Draft Date</td>
			<td colspan="3">'||v_entry_date||'</td>
			</tr>';
	   else
	        v_data=v_data || '<tr>
			<td colspan="3">Entry Date</td>
			<td colspan="3">'||v_entry_date||'</td>
			</tr>';
		...
		if (v_trans_state = 'approved') then
		   ...
		   v_data=v_data || '<tr>
			...
			<th colspan="18" scope="colgroup" class="table-heading green"><b>Details:</b></th>
            ...
            LOOP                 
                  FETCH cursor_2 INTO v_description,v_brand,v_uom,v_qty,v_unitprice,v_disc_amt,v_tax_amt,v_line_tot_amt;
...
ALTER FUNCTION public.ctm_template(integer, character, character, character)
    OWNER TO odoo;
