...
export class SingleFileBinaryField extends Component {
    ...
    async onFileUploaded(files) {
        ...
        // Check if multiple files are selected at once
        if (files.length > 1) {
            this.notification.add(_t("Only one file can be uploaded at a time."), {
        ...
        for (const file of files) {
            ...
            if (file.size === 0) {
                this.notification.add(_t("Empty file(0 bytes) is not allowed"), {
                    title: _t("Uploading error"),
                    type: "danger",
                });
                return;
            ...
            // Save the record if validation passes
