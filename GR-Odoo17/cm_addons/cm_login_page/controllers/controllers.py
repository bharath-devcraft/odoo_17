...
class Home(http.Controller):
    ...
    def custom_page(self, **kw):
        return request.render('cm_login_page.custom_page_template')
    
    @http.route('/forget_password_odoo', type='http', auth='public', website=True, sitemap=False, multilang=False)
    def custom_404(self, **post):
        return request.render('cm_login_page.forget_password_odoo')
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if style == 'default' or style is False:
            ...
            response = request.render('cm_login_page.left_login_template', values)
        elif style == 'right':
            response = request.render('cm_login_page.right_login_template', values)
        else:
            response = request.render('cm_login_page.middle_login_template', values)
